...
typedef struct AVFilmGrainAOMParams
{
    ...
    /**
     * Chroma auto-regression coefficients. The number of coefficients is given
     * by 2 * ar_coeff_lag * (ar_coeff_lag + 1) + !!num_y_points.
     */
...
typedef struct AVFilmGrainH274Params
{
    ...
    /**
     * Specifies a scale factor used in the film grain characterization
     * equations.
     */
    int log2_scale_factor;

    /**
     * Indicates if the modelling of film grain for a given component is
     * present.
     */
     ...
     */
    uint8_t intensity_interval_lower_bound[3 /* y, cb, cr */]
                                          [256 /* intensity interval */];

    /**
     * Specifies the upper bound of each intensity interval for which the set of
     * model values applies for the component.
     */
    uint8_t intensity_interval_upper_bound[3 /* y, cb, cr */]
                                          [256 /* intensity interval */];

    /**
     * Specifies the model values for the component for each intensity interval.
     * - When model_id == 0, the following applies:
     *     For comp_model_value[y], the range of values is [0, 2^bit_depth_luma
     * - 1] For comp_model_value[cb..cr], the range of values is [0,
     * 2^bit_depth_chroma - 1]
     * - Otherwise, the following applies:
     *     For comp_model_value[y], the range of values is [-2^(bit_depth_luma -
     * 1), 2^(bit_depth_luma - 1) - 1] For comp_model_value[cb..cr], the range
     * of values is [-2^(bit_depth_chroma - 1), 2^(bit_depth_chroma - 1) - 1]
     */
    int16_t comp_model_value[3 /* y, cb, cr */][256 /* intensity interval */]
                            [6 /* model value */];
