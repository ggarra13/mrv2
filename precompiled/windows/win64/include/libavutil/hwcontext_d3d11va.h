...
typedef struct AVD3D11VAFramesContext
{
     ...
     * ID3D11VideoDecoderOutputView objects for each array texture element on
    ...
    /**
     * In case if texture structure member above is not NULL contains the same
     * texture pointer for all elements and different indexes into the array
     * texture. In case if texture structure member above is NULL, all elements
     * contains pointers to separate non-array textures and 0 indexes. This
     * field is ignored/invalid if a user-allocated texture is provided.
     */
