 ...
 * allocated with swr_alloc() or swr_alloc_set_opts2(). It is opaque, so all
 * parameters must be set with the @ref avoptions API.
 *
 * The first thing you will need to do in order to use lswr is to allocate
 * SwrContext. This can be done with swr_alloc() or swr_alloc_set_opts2(). If
 * you are using the former, you must set options through the @ref avoptions
 * API. The latter function provides the same feature, but it allows you to set
 ...
 * out_ch_layout AV_SAMPLE_FMT_S16,    // out_sample_fmt 44100, //
 * out_sample_rate
 *                       &(AVChannelLayout)AV_CHANNEL_LAYOUT_5POINT1, //
 * in_ch_layout AV_SAMPLE_FMT_FLTP,   // in_sample_fmt 48000,                //
 * in_sample_rate 0,                    // log_offset NULL);                //
 * log_ctx
 * @endcode
 *
 ...
 */
attribute_deprecated struct SwrContext* swr_alloc_set_opts(
 ...
 * on the allocated context.
 *
 * @param ps              Pointer to an existing Swr context if available, or to
 * NULL if not. On success, *ps will be set to the allocated context.
 ...
 */
int swr_convert(
    struct SwrContext* s, uint8_t** out, int out_count, const uint8_t** in,
    int in_count);
...
int swr_set_compensation(
    struct SwrContext* s, int sample_delta, int compensation_distance);
 ...
 */
attribute_deprecated int swr_build_matrix(
    uint64_t in_layout, uint64_t out_layout, double center_mix_level,
    double surround_mix_level, double lfe_mix_level, double rematrix_maxval,
...
int swr_build_matrix2(
    const AVChannelLayout* in_layout, const AVChannelLayout* out_layout,
    double center_mix_level, double surround_mix_level, double lfe_mix_level,
    double maxval, double rematrix_volume, double* matrix, ptrdiff_t stride,
    enum AVMatrixEncoding matrix_encoding, void* log_context);
