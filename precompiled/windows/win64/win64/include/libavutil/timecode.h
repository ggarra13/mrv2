 ...
 * <baptiste.coudurier@gmail.com> Copyright (c) 2011-2012 Smartjog S.A.S,
...
enum AVTimecodeFlag {
    ...
    AV_TIMECODE_FLAG_ALLOWNEGATIVE = 1
                                     << 2, ///< negative time values are allowed
};
...
uint32_t av_timecode_get_smpte(
    AVRational rate, int drop, int hh, int mm, int ss, int ff);
...
char* av_timecode_make_smpte_tc_string2(
    char* buf, AVRational rate, uint32_t tcsmpte, int prevent_df,
    int skip_field);
...
char* av_timecode_make_smpte_tc_string(
    char* buf, uint32_t tcsmpte, int prevent_df);
...
int av_timecode_init(
    AVTimecode* tc, AVRational rate, int flags, int frame_start, void* log_ctx);
...
int av_timecode_init_from_components(
    AVTimecode* tc, AVRational rate, int flags, int hh, int mm, int ss, int ff,
    void* log_ctx);
...
int av_timecode_init_from_string(
    AVTimecode* tc, AVRational rate, const char* str, void* log_ctx);
