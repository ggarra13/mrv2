...
void Scroll::clear() {
  ...
  // and deletion. Finally they are added to Scroll's group again. This
...
void Scroll::fix_scrollbar_order() {
...
/**
  ...
  \since FLTK 1.4.0
*/
int Scroll::delete_child(int index) {
...
//
void Scroll::draw_clip(void* v,int X, int Y, int W, int H) {
...
/**
  ...
  \see bbox()
*/
void Scroll::recalc_scrollbars(ScrollInfo &si) const {
...
*/
void Scroll::bbox(int& X, int& Y, int& W, int& H) const {
  ScrollInfo si;
...
/**
  Resizes the Scroll widget and moves its children if necessary.

  The Scroll widget first resizes itself, and then it moves all its
  children if (and only if) the Scroll widget has been moved. The
  children are moved by the same amount as the Scroll widget has been
  moved, hence all children keep their relative positions.

  \note Scroll::resize() does \b not call Fl_Group::resize(), and
  child widgets are \b not resized.

  Since children of an Scroll are not resized, the resizable() widget
  is ignored (if it is set).

  The scrollbars are moved to their proper positions, as given by
  Scroll::scrollbar.align(), and switched on or off as necessary.

  \note Due to current (FLTK 1.3.x) implementation constraints some of this
  may effectively be postponed until the Scroll is drawn the next time.
...
/**  Moves the contents of the scroll group to a new position.
  ...
  This is like moving the scrollbars of the Scroll around. For instance:
  \code
    Scroll scroll (10,10,200,200);
  ...
  relative to the origin of the Scroll (10,10), i.e. Fl_Box b3 will
  be visible in the top left corner of the scroll area.
*/
void Scroll::scroll_to(int X, int Y) {
  int dx = xposition_-X;
  int dy = yposition_-Y;
  if (!dx && !dy) return;
  xposition_ = X;
...
/**
  ...
  Scroll and all of its children can be automatic (local)
  variables, but you must declare the Scroll <I>first</I>, so
