...
std::string GetCpuCaps( CpuCaps *caps)
{
...
#if defined(WIN32) || defined(WIN64) || defined(__linux__) || defined(__FreeBSD__) || defined(__NetBSD__) || defined(__CYGWIN__) || defined(__OpenBSD__) || defined(__DragonFly__) || defined(__APPLE__)
  if (caps->hasSSE)
  ...
  //
  if ( gCpuCaps.hasMMX      ) out << "MMX ";
  if ( gCpuCaps.hasMMX2     ) out << "MMX2 ";
  if ( gCpuCaps.has3DNow    ) out << "3DNow ";
...
static void check_os_katmai_support( void )
{
  ...
  if ( gCpuCaps.hasSSE ) {
    ...
    if ( gCpuCaps.hasSSE ) LOG_VERBOSE("yes." );
    else LOG_VERBOSE("no!" );
  }
