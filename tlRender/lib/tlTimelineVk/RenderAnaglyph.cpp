...
#define RIGHT_IMAGE 1
            ...
            // Prepare shaders
            //
            auto overlayShader = p.shaders["overlay"];
            _createBindingSet(overlayShader);
            color = image::Color4f(1.F, 1.F, 1.F);
            vkCmdPushConstants(p.cmd, pipelineLayout,
                               overlayShader->getPushStageFlags(), 0,
                               sizeof(color), &color);
            overlayShader->setUniform("transform.mvp", p.transform, vlk::kShaderVertex);
            overlayShader->setFBO("textureSampler", p.buffers["stereo_image"]);
            _bindDescriptorSets(pipelineLayoutName, overlayShader);
