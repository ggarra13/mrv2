...
namespace
{
    ...
    VkDeviceMemory allocateAndBindImageMemory(VkPhysicalDevice gpu,
                                              VkDevice device,
                                              VkImage image)
    {
    ...
    void NDIView::endSingleTimeCommands(VkDevice device,
                                        VkQueue queue,
...
namespace tl
{
    namespace timeline_vk
    {
            ...
            const std::shared_ptr<TextureCache>& textureCache)
        {
            ...
            p.glyphTextureAtlas = vk::TextureAtlas::create(ctx, 
        ...
        std::shared_ptr<Render> Render::create(ctx, 
            const std::shared_ptr<system::Context>& context,
            const std::shared_ptr<TextureCache>& textureCache)
        {
            ...
            const timeline::RenderOptions& renderOptions)
        {
            ...
            {
                p.shaders["rect"] =
                    vk::Shader::create(ctx, vertexSource(), meshFragmentSource());
            }
            if (!p.shaders["mesh"])
            {
                p.shaders["mesh"] =
            ...
            {
                p.shaders["colorMesh"] = vk::Shader::create(ctx, 
                    colorMeshVertexSource(), colorMeshFragmentSource());
            }
            if (!p.shaders["text"])
            {
                p.shaders["text"] =
                    vk::Shader::create(ctx, vertexSource(), textFragmentSource());
            }
            if (!p.shaders["texture"])
            {
                p.shaders["texture"] =
                    vk::Shader::create(ctx, vertexSource(), textureFragmentSource());
            }
            if (!p.shaders["image"])
            {
                p.shaders["image"] =
                    vk::Shader::create(ctx, vertexSource(), imageFragmentSource());
            }
            if (!p.shaders["wipe"])
            {
                p.shaders["wipe"] =
            ...
            {
                p.shaders["difference"] = vk::Shader::create(ctx, 
                ...
                const pl_shader_res* res)
            {
                ...
                for (unsigned i = 0; i < res->num_descriptors; ++i)
                {
                    ...
                    case PL_DESC_SAMPLED_TEX:
                    {
                        ...
                        VkDeviceMemory imageMemory = allocateAndBindImageMemory(ctx->gpu, ctx->device, image);
        ...
        void Render::setOCIOOptions(const timeline::OCIOOptions& value)
        {
            ...
            if (p.ocioOptions.enabled)
            {
                ...
                p.ocioData->transform = OCIO::DisplayViewTransform::Create(Ctx, );
                    ...
                    !p.ocioOptions.view.empty())
                {
                    ...
                    p.ocioData->lvp = OCIO::LegacyViewingPipeline::Create(Ctx, );
                    if (!p.ocioData->lvp)
                    {
                        p.ocioData.reset();
                        throw std::runtime_error(
