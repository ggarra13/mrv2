    ...
    namespace timeline_vlk
    {
        ...
        Render::drawRect(const math::Box2i& box, const image::Color4f& color)
        {
            ...
            if (p.vaos["rect"])
            {
                ...
                createPipeline(p.fbo, pipelineName, shaderName, meshName,
                               enableBlending);
                bindDescriptorSets(pipelineName, shaderName);

                VkPipelineLayout pipelineLayout = p.pipelineLayouts["timeline_rect"];
                vkCmdPushConstants(p.cmd, pipelineLayout,
                                   p.shaders["rect"]->getPushStageFlags(),
            ...
            const image::Color4f& color)
        {
            ...
            if (size > 0)
            {
                ...
                if (p.vbos["mesh"])
                {
                    p.vbos["mesh"]->copy(convert(mesh, vlk::VBOType::Pos2_F32_UV_U16));
                }
                ...
                if (p.vaos["mesh"] && p.vbos["mesh"])
                {
                    ...
                    VkPipelineLayout pipelineLayout = p.pipelineLayouts["timeline_mesh"];
                    vkCmdPushConstants(p.cmd, pipelineLayout,
                                       p.shaders["mesh"]->getPushStageFlags(),
                                       0, sizeof(color), &color);
            ...
            const math::Vector2i& position, const image::Color4f& color)
        {
            ...
            if (size > 0)
            {
                ...
                {
                    const std::string pipelineLayoutName = pipelineName + "_colorMesh";
                    VkPipelineLayout pipelineLayout = p.pipelineLayouts[pipelineLayoutName];
                    vkCmdPushConstants(p.cmd, pipelineLayout,
                                       p.shaders["colorMesh"]->getPushStageFlags(),
                                       0, sizeof(color), &color);
            ...
            const VkBlendOp alphaBlendOp)
        {
            ...
            else
            {
                ...
                {   
                    ...
                    {
                        p.garbage[p.frameIndex].pipelines.push_back(oldPipeline);
                    }
                    pipeline = pipelineState.create(device);
                    auto pair = std::make_pair(pipelineState, pipeline);
                    p.pipelines[pipelineName] = pair;
