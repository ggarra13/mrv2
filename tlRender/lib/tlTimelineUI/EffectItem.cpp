...
namespace tl
{
    namespace TIMELINEUI
    {
        ...
        std::shared_ptr<EffectItem> EffectItem::create(
            ...
            const std::shared_ptr<IWidget>& parent)
        {
            auto out = std::shared_ptr<EffectItem>(new EffectItem);
            out->_init(
                effect, item, scale, options, displayOptions, itemData, context,
                parent);
            return out;
        }

        void EffectItem::setDurationLabel(const std::string& value)
        {
        ...
        void
        EffectItem::clipEvent(const math::Box2i& clipRect, bool clipped)
        {
        ...
        void EffectItem::_timeUnitsUpdate()
        {
            IItem::_timeUnitsUpdate();
            _textUpdate();
        }

        void EffectItem::_textUpdate()
        {
            TLRENDER_P();
