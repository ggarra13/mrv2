...
namespace tl
{
    namespace vlk
    {
        ...
        {
            // 16 MB
            constexpr size_t DYNAMIC_VERTEX_BUFFER_SIZE = 32 * 1024 * 1024;
        }
        ...
        struct VAO::Private
        {
            ...
            std::vector<UploadRegion> uploads[MAX_FRAMES_IN_FLIGHT];
        };
        ...
        void VAO::bind(uint32_t value)
        {
            ...
            // New frame partition: reset relative offset and clear upload records
            p.frameIndex = value;
            p.relativeOffset = 0;
            p.uploads[p.frameIndex].clear();
        }
        ...
        void VAO::upload(const std::vector<uint8_t>& vertexData)
        {
            ...
            if (p.relativeOffset + dataSize > p.regionSize)
                throw std::runtime_error("Per-frame vertex partition overflow.");

            // Compute absolute offset in the big buffer: per-frame region + relative
            size_t absOffset = p.frameIndex * p.regionSize + p.relativeOffset;
            ...
            if (p.relativeOffset < copied)
                throw std::runtime_error("Overwriting buffer with relativeOffset");
            
            // Flush the mapped memory to ensure GPU visibility
            // Gemini: is this needed?  We mapped it with
            //         VK_MEMORY_PROPERTY_HOST_COHERENT_BIT
            
            // VkDeviceSize flushOffset = absOffset;
            // VkDeviceSize flushSize   = ((dataSize + p.alignment - 1) / p.alignment) * p.alignment;
            // if (dataSize > flushSize)
            //     throw std::runtime_error("flushSize < dataSize");

            // // Flush the mapped memory to ensure GPU visibility
            // // Gemini: is this needed?  We mapped it with
            // //         VK_MEMORY_PROPERTY_HOST_COHERENT_BIT
            // //         and we have in our FLTK Window a frame resources
            // //         with a VkFence for the swapchain
            ...
            p.uploads[p.frameIndex].push_back({ absOffset, dataSize });

        }

        void VAO::draw(VkCommandBuffer& cmd, std::size_t vertexCount)
        {
            TLRENDER_P();
            auto& regs = p.uploads[p.frameIndex];
            if (regs.empty())
                throw std::runtime_error("No upload to draw in VAO::draw");

            const auto& upload = regs.back();
            VkDeviceSize offset = upload.offset;

            
            // VkBufferMemoryBarrier bufferBarrier{};
            // bufferBarrier.sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER;
            // bufferBarrier.buffer = p.vertexBuffer;
            // bufferBarrier.offset = p.frameIndex * p.regionSize; // Barrier covers the specific frame's region
            // bufferBarrier.size = p.relativeOffset; // Barrier covers the actual data written this frame
            // bufferBarrier.srcAccessMask = VK_ACCESS_HOST_WRITE_BIT;
            // bufferBarrier.dstAccessMask = VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT;
            // bufferBarrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED; // Assuming single queue
            // bufferBarrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED; // Assuming single queue

            // vkCmdPipelineBarrier(
            //     cmd,
            //     VK_PIPELINE_STAGE_HOST_BIT,           // Data was written on the host
            //     VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,   // Vertex buffer is read by vertex input
            //     0,                                    // No dependency flags
            //     0, nullptr,                         // No general memory barriers
            //     1, &bufferBarrier,                  // One buffer memory barrier
