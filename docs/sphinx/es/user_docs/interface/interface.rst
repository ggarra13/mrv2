...
.. topic:: Debanding (Vulkan only)

   Con esta opcion puede suavizar áreas de la imagen que tienen  dients by running a localized blur filter.  It is useful mainly for video files which are susceptible to banding.  However, for professional video work when seeing images, it should probably be turned off.
...
.. topic:: Transitiones
   ...
   archivos .otio.
   (Actualmente no implementado en v1.5.4).
