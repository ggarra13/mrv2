...
cmd="cmake -G '${CMAKE_GENERATOR}' -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -D CMAKE_INSTALL_PREFIX=$PWD/install -D CMAKE_PREFIX_PATH=$PWD/install -D TLRENDER_USD=${TLRENDER_USD} -D TLRENDER_VPX=ON -D TLRENDER_YASM=ON -D TLRENDER_RAW=ON -D TLRENDER_NFD=OFF -D TLRENDER_WAYLAND=${TLRENDER_WAYLAND} -D TLRENDER_NET=${TLRENDER_NET} -D TLRENDER_PROGRAMS=OFF -D TLRENDER_EXAMPLES=FALSE -D TLRENDER_TESTS=FALSE -D TLRENDER_QT6=OFF -D TLRENDER_QT5=OFF ${CMAKE_FLAGS} ../.."
