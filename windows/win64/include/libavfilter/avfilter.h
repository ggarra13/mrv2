...
typedef struct AVFilter
{
    ...
    union
    {
         ...
         * inputs and outputs are fixed), shortly before the format negotiation.
         ...
         * filter supports on that link. For audio links, this filter must also
         * set
         * @ref AVFilterFormatsConfig.samplerates "incfg.samplerates"
         *  /
         * @ref AVFilterFormatsConfig.samplerates "outcfg.samplerates"
         * and @ref AVFilterFormatsConfig.channel_layouts
     ...
     * @param res    a buffer with size res_size where the filter(s) can return
     * a response. This must not change when the command is not supported.
     * @param flags  if AVFILTER_CMD_FLAG_FAST is set and the command would be
     *               time consuming then a filter should treat it like an
     * unsupported command
     *
     ...
     */
    int (*process_command)(
        AVFilterContext*, const char* cmd, const char* arg, char* res,
 ...
 */
int avfilter_link(
    AVFilterContext* src, unsigned srcpad, AVFilterContext* dst,
    unsigned dstpad);
...
#define AVFILTER_CMD_FLAG_ONE                                                  \
    1 ///< Stop once a filter understood the command (for target=all for
      ///< example), fast filters are favored automatically
#define AVFILTER_CMD_FLAG_FAST                                                 \
    2 ///< Only execute command when its fast (like a video out that supports
 ...
 *
 * @param opaque a pointer where libavfilter will store the iteration state.
...
int avfilter_insert_filter(
    AVFilterLink* link, AVFilterContext* filt, unsigned filt_srcpad_idx,
...
typedef int(avfilter_execute_func)(
    AVFilterContext* ctx, avfilter_action_func* func, void* arg, int* ret,
    int nb_jobs);
...
typedef struct AVFilterGraph
{
    ...
    char* aresample_swr_opts; ///< swr options to use for the auto-inserted
                              ///< aresample filters, Access ONLY through
                              ///< AVOptions
...
AVFilterContext* avfilter_graph_alloc_filter(
    AVFilterGraph* graph, const AVFilter* filter, const char* name);
...
int avfilter_graph_create_filter(
    AVFilterContext** filt_ctx, const AVFilter* filt, const char* name,
...
int avfilter_graph_parse(
    AVFilterGraph* graph, const char* filters, AVFilterInOut* inputs,
 ...
 * NULL. If non-NULL, *inputs is updated to contain the list of open inputs
 *                after the parsing, should be freed with avfilter_inout_free().
 * @param outputs pointer to a linked list to the outputs of the graph, may be
 * NULL. If non-NULL, *outputs is updated to contain the list of open outputs
 *                after the parsing, should be freed with avfilter_inout_free().
 ...
 *                     parsed graph will be returned here. It is to be freed by
 * the caller using avfilter_inout_free().
 * @return zero on success, a negative AVERROR code on error
 *
...
typedef struct AVFilterParams
{
     ...
     * with an equivalent of av_opt_set_dict2(filter, &opts,
     * AV_OPT_SEARCH_CHILDREN), i.e. any unapplied options will be left in this
     * dictionary.
     */
    AVDictionary* opts;
...
int avfilter_graph_segment_parse(
    AVFilterGraph* graph, const char* graph_str, int flags,
...
int avfilter_graph_segment_link(
    AVFilterGraphSegment* seg, int flags, AVFilterInOut** inputs,
    AVFilterInOut** outputs);

/**
 * Apply all filter/link descriptions from a graph segment to the associated
 * filtergraph.
 *
 ...
 * @param cmd    the command to send, for handling simplicity all commands must
 * be alphanumeric only
 * @param arg    the argument for the command
 * @param res    a buffer with size res_size where the filter(s) can return a
 * response.
 *
 * @returns >=0 on success otherwise an error code.
 *              AVERROR(ENOSYS) on unsupported commands
...
int avfilter_graph_send_command(
    AVFilterGraph* graph, const char* target, const char* cmd, const char* arg,
 ...
 *               otherwise it can be a filter or filter instance name
 *               which will send the command to all matching filters.
 * @param cmd    the command to sent, for handling simplicity all commands must
 * be alphanumeric only
 * @param arg    the argument for the command
