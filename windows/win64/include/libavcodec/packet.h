...
enum AVPacketSideDataType {
     ...
     * struct, usually exported by some encoders (on demand through the prft
     * flag set in the AVCodecContext export_side_data field).
     */
    AV_PKT_DATA_PRFT,
...
uint8_t* av_packet_new_side_data(
    AVPacket* pkt, enum AVPacketSideDataType type, size_t size);
...
int av_packet_add_side_data(
    AVPacket* pkt, enum AVPacketSideDataType type, uint8_t* data, size_t size);
...
uint8_t* av_packet_get_side_data(
    const AVPacket* pkt, enum AVPacketSideDataType type, size_t* size);
...
int av_packet_unpack_dictionary(
    const uint8_t* data, size_t size, AVDictionary** dict);
