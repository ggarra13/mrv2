...
#define AV_NUM_DETECTION_BBOX_CLASSIFY 4
    ...
    char classify_labels[AV_NUM_DETECTION_BBOX_CLASSIFY]
...
/*
 * Get the bounding box at the specified {@code idx}. Must be between 0 and
 * nb_bboxes.
 */
...
/**
 * Allocates memory for AVDetectionBBoxHeader, plus an array of {@code
 * nb_bboxes} AVDetectionBBox, and initializes the variables. Can be freed with
 * a normal av_free() call.
 *
 ...
 */
AVDetectionBBoxHeader*
av_detection_bbox_alloc(uint32_t nb_bboxes, size_t* out_size);

/**
 * Allocates memory for AVDetectionBBoxHeader, plus an array of {@code
 * nb_bboxes} AVDetectionBBox, in the given AVFrame {@code frame} as
 * AVFrameSideData of type AV_FRAME_DATA_DETECTION_BBOXES and initializes the
 * variables.
 */
AVDetectionBBoxHeader*
av_detection_bbox_create_side_data(AVFrame* frame, uint32_t nb_bboxes);
#endif
