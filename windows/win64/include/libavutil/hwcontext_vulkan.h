...
typedef struct AVVulkanDeviceContext
{
     ...
     * enabled during device creation. When a device is created by FFmpeg, it
     * will default to enabling all that are present of the
     * shaderImageGatherExtended, fragmentStoresAndAtomics, shaderInt64 and
     ...
     * with each entry containing the specified Vulkan extension string to
     * enable. Duplicates are possible and accepted. If no extensions are
     * enabled, set these fields to NULL, and 0 respectively.
     */
     ...
     * VK_KHR_external_semaphore_fd and VK_EXT_external_memory_host are enabled
     * if found. If supplying your own device context, these fields takes the
     * same format as the above fields, with the same conditions that duplicates
     * are possible and accepted, and that NULL and 0 respectively means no
    ...
    /**
     * Queue family index for video encode ops, and the amount of queues
     * enabled. If the device doesn't support such, queue_family_encode_index
     * will be -1. Not required.
     */
    int queue_family_encode_index;
    int nb_encode_queues;

    /**
     * Queue family index for video decode ops, and the amount of queues
     * enabled. If the device doesn't support such, queue_family_decode_index
     * will be -1. Not required.
     */
...
/**
 * Returns the format of each image up to the number of planes for a given
 * sw_format. Returns NULL on unsupported formats.
 */
