 ...
 * error
 */
av_warn_unused_result int sws_init_context(
 ...
 */
int sws_send_slice(
    struct SwsContext* c, unsigned int slice_start, unsigned int slice_height);
 ...
 *                     frame height)
 *
 * @return a non-negative number if the data was successfully written into the
 * output AVERROR(EAGAIN) if more input data needs to be provided before the
 ...
 *
 * @param c   The scaling context
 * @return alignment required for output slices requested with
 * sws_receive_slice(). Slice offsets and sizes passed to sws_receive_slice()
 * must be multiples of the value returned from this function.
 */
...
int sws_setColorspaceDetails(
    struct SwsContext* c, const int inv_table[4], int srcRange,
    const int table[4], int dstRange, int brightness, int contrast,
    int saturation);
...
int sws_getColorspaceDetails(
    struct SwsContext* c, int** inv_table, int* srcRange, int** table,
...
struct SwsContext* sws_getCachedContext(
    struct SwsContext* context, int srcW, int srcH,
...
void sws_convertPalette8ToPacked32(
    const uint8_t* src, uint8_t* dst, int num_pixels, const uint8_t* palette);
