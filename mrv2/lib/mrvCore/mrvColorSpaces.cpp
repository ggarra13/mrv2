 ...
 */
inline float hue(const Color4f &rgb, const float maxV,
                 const float spanV) noexcept {
  float h;
  if (rgb.r == maxV) {
    h = (rgb.g - rgb.b) / spanV;
  } else if (rgb.g == maxV) {
...
Color4f to_xyz(const Color4f &rgb, const Imf::Chromaticities &chroma,
...
Color4f to_xyY(const Color4f &rgb, const Imf::Chromaticities &chroma,
               const float Y) noexcept {
...
Color4f to_lab(const Color4f &rgb, const Imf::Chromaticities &chroma,
               const float Y) noexcept {
...
Color4f to_luv(const Color4f &rgb, const Imf::Chromaticities &chroma,
               const float Y) noexcept {
...
Color4f to_yiq(const Color4f &rgb) noexcept {
  return Color4f(rgb.r * 0.299f + rgb.g * 0.587f + rgb.b * 0.114f,
...
Color4f to_ITU601(const Color4f &rgb) noexcept {
  return Color4f(16.f + rgb.r * 65.481f + rgb.g * 128.553f + rgb.b * 24.966f,
...
Color4f to_rgb(const Color4f &yuv) noexcept {
  ...
  Color4f rgb(y2 + 2.018f * (yuv.g - 128),
...
namespace YPbPr {
imaging::Color4f to_rgb(const imaging::Color4f &YPbPr,
