...
char *GetCpuFriendlyName(std::ostringstream &out, unsigned int regs[],
                         unsigned int regs2[]);
...
std::string GetCpuCaps(CpuCaps *caps) {
  ...
  char buf[1024];
  snprintf(buf, 1023, "CPU vendor name: %x %x %x %x", regs[1], regs[3], regs[2],
           regs[0]);
  LOG_VERBOSE(buf);
  ...
  if (regs[0] >= 0x00000001) {
    ...
    LOG_INFO("(Family: " << caps->cpuType << ", Model: " << caps->cpuModel
...
char *GetCpuFriendlyName(std::ostringstream &out, unsigned int regs[],
  ...
  snprintf(vendor, 13, "%.4s%.4s%.4s", (char *)(regs + 1), (char *)(regs + 3),
           (char *)(regs + 2));
