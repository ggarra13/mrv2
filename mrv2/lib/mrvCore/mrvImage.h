...
namespace mrv
{
    ...
    inline
    void flipImageInY(uint8_t* outputPixels,
                      const uint8_t* inputPixels, const size_t width, const size_t height, const int depth)
    {
        ...
        for (size_t y = 0; y < height ; ++y) {
            // Calculate the indices of the rows in source and destination buffers
            size_t topInputRow = y * rowSize;
            size_t bottomInputRow = (height - y - 1) * rowSize;
            size_t topOutputRow = (height - y - 1) * rowSize; // Start from bottom in output

            // Copy a row from top to bottom in the output buffer
            memcpy(outputPixels + topOutputRow, inputPixels + topInputRow, rowSize);
        }
    }
