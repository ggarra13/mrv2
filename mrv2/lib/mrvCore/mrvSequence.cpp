...
std::string get_short_view(bool left) {
  ...
  if (idx == std::string::npos) {
    // LOG_ERROR( "MRV_STEREO_CHAR_PAIRS does not have two letters separated by
    // colon" );
    if (left)
      return "L";
    else
...
std::string get_long_view(bool left) {
  ...
  if (idx == std::string::npos) {
    LOG_ERROR(
        "MRV_STEREO_NAME_PAIRS does not have two names separated by colon");
    if (left)
      return "left";
    else
      return "right";
...
bool split_sequence(std::string &root, std::string &frame, std::string &view,
  ...
  if (periods.size() == 4) {
    ...
    if (!view.empty())
      view += ".";
    if (mrv::is_valid_movie(ext.c_str()) || mrv::is_valid_audio(ext.c_str())) {
    ...
    if (mrv::is_valid_movie(ext.c_str()) || mrv::is_valid_audio(ext.c_str())) {
      ...
      if (!mrv::is_valid_frame(frame) && !mrv::is_valid_frame_spec(frame) &&
  ...
  if (count == 2 && minus < 2) {
    ...
    if (ok &&
        (!is_valid_movie(ext.c_str()) || mrv::is_valid_audio(ext.c_str()))) {
      return true;
    }
...
bool get_sequence_limits(int64_t &frameStart, int64_t &frameEnd,
                         std::string &fileroot, const bool error) {
  ...
  if ((!fs::exists(dir)) || (!fs::is_directory(dir))) {
    if (error) {
      std::string err =
          tl::string::Format(_("Directory {0} does not exist or no directory"))
              .arg(dir);
      LOG_ERROR(err);
    }
    return false;
...
bool fileroot(std::string &fileroot, const std::string &file,
  ...
  bool ok =
      split_sequence(root, frame, view, ext, file, change_view, change_frame);
  ...
  char full[1024];
  if (pad == 0) {
    snprintf(full, 1024, "%s%s%%%s%s", root.c_str(), view.c_str(), digits,
             ext.c_str());
  } else {
    snprintf(full, 1024, "%s%s%%0%d%s%s", root.c_str(), view.c_str(), pad,
             digits, ext.c_str());
  }
...
void parse_directory(const std::string &dir, stringArray &movies,
