...
struct App::Private {
  ...
  std::shared_ptr<observer::ListObserver<std::shared_ptr<FilesModelItem>>>
      activeObserver;
  std::shared_ptr<observer::ListObserver<std::shared_ptr<FilesModelItem>>>
      allObserver;
...
App::App(int argc, char **argv, const std::shared_ptr<system::Context> &context)
    : IApp(), _p(new Private) {
  TLRENDER_P();
  ...
  IApp::_init(
      ...
      {app::CmdLineValueOption<int>::create(
       ...
       app::CmdLineValueOption<double>::create(p.options.speed, {"-speed"},
                                               "Playback speed."),
       ...
       app::CmdLineValueOption<timeline::LUTOrder>::create(
           p.options.lutOptions.order, {"-lutOrder"}, "LUT operation order.",
           string::Format("{0}").arg(p.options.lutOptions.order),
           string::join(timeline::getLUTOrderLabels(), ", ")),
       app::CmdLineFlagOption::create(p.options.resetSettings,
                                      {"-resetSettings"},
  ...
  p.layersObserver = observer::ListObserver<int>::create(
      p.filesModel->observeLayers(), [this](const std::vector<int> &value) {
        DBG;
        for (size_t i = 0; i < value.size() && i < _p->timelinePlayers.size();
             ++i) {
        ...
        for (const auto &i : value) {
          ...
          case log::Type::Error:
            ...
            _p->ui->uiStatusBar->copy_label(
                std::string(string::Format(_("ERROR: {0}")).arg(i.message))
                    .c_str());
            break;
          default:
            break;
  ...
  p.devicesObserver = observer::ValueObserver<DevicesModelData>::create(
      p.devicesModel->observeData(), [this](const DevicesModelData &value) {
  ...
  if (!p.options.fileName[0].empty()) {
    ...
    for (int i = 2; i >= 0; --i) {
      if (p.options.fileName[i].empty())
        continue;
      open(p.options.fileName[i].c_str(), p.options.audioFileName.c_str());
    }
    ...
    if (!p.timelinePlayers.empty() && p.timelinePlayers[0]) {
      ...
      c->uiTimeline->setColorConfigOptions(p.options.colorConfigOptions);
    }
  } else {
    const auto fps = 24.0;
    ...
    c->uiEndFrame->setTime(startTime + duration -
...
App::~App() {
  ...
  if (p.settingsObject && p.devicesModel) {
    ...
    p.settingsObject->setValue("Devices/DeviceIndex",
                               static_cast<int>(deviceData.deviceIndex));
    p.settingsObject->setValue("Devices/DisplayModeIndex",
                               static_cast<int>(deviceData.displayModeIndex));
    p.settingsObject->setValue("Devices/PixelTypeIndex",
                               static_cast<int>(deviceData.pixelTypeIndex));
    p.settingsObject->setValue("Devices/HDRMode",
...
const timeline::LUTOptions &App::lutOptions() const { return _p->lutOptions; }
...
void App::open(const std::string &fileName, const std::string &audioFileName) {
  ...
  pathOptions.maxNumberDigits =
      std_any_cast<int>(p.settingsObject->value("Misc/MaxFileSequenceDigits"));
...
void App::_activeCallback(
  ...
  DBG;
  if (!p.active.empty() && !p.timelinePlayers.empty() && p.timelinePlayers[0]) {
  ...
  for (const auto &i : items) {
    ...
    if (it != p.itemsMapping.end()) {
      ...
      auto ip = std::find(newTimelinePlayers.begin(), newTimelinePlayers.end(),
                          player);
    ...
    try {
      ...
      int value =
          std_any_cast<int>(p.settingsObject->value("FileSequence/Audio"));
      ...
      options.ioOptions["SequenceIO/ThreadCount"] =
          string::Format("{0}").arg(std_any_cast<int>(
              p.settingsObject->value("Performance/SequenceThreadCount")));
      ...
      auto otioTimeline =
          i->audioPath.isEmpty()
              ...
              : timeline::create(i->path.get(), i->audioPath.get(), _context,
                                 options);
      ...
      if (info.video.empty())
        throw std::runtime_error(
            string::Format("{0}: Error reading file").arg(i->path.get()));
      ...
      value =
          std_any_cast<int>(p.settingsObject->value("Performance/TimerMode"));
  ...
  DBG;
  if (!items.empty() && !newTimelinePlayers.empty() && newTimelinePlayers[0]) {
  ...
  if (p.ui) {
    ...
    if (!p.timelinePlayers.empty()) {
      ...
      int idx =
          c->uiAudioTracks->add(name.c_str(), 0, 0, 0, mode | FL_MENU_VALUE);
...
otime::RationalTime App::_cacheReadAhead() const {
  ...
  return otime::RationalTime(value / static_cast<double>(activeCount), 1.0);
}
