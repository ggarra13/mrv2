...
OpenSeparateAudioDialog::OpenSeparateAudioDialog(
    ...
    : Fl_Window(540, 240, _("Open with Audio")), _p(new Private) {
  TLRENDER_P();
...
void OpenSeparateAudioDialog::_browseVideoCallback_cb(Fl_Widget *w,
                                                      void *data) {
  OpenSeparateAudioDialog *self = static_cast<OpenSeparateAudioDialog *>(data);
  self->_browseVideoCallback(w, data);
}

void OpenSeparateAudioDialog::_browseAudioCallback_cb(Fl_Widget *w,
                                                      void *data) {
  OpenSeparateAudioDialog *self = static_cast<OpenSeparateAudioDialog *>(data);
