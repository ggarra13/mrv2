...
FilesModel::FilesModel() : _p(new Private) {}
...
void FilesModel::close() {
  ...
  if (p.a->get()) {
    ...
    if (i != files.end()) {
      ...
      const int aNewIndex =
          math::clamp(aPrevIndex, 0, static_cast<int>(files.size()) - 1);
...
void FilesModel::setLayer(const std::shared_ptr<FilesModelItem> &item,
                          int layer) {
  TLRENDER_P();
