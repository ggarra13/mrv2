...
ColorPanel::ColorPanel(ViewerUI *ui)
    : colorOn(nullptr), levelsOn(nullptr), softClipOn(nullptr),
      PanelWidget(ui) {
  add_group("Color");
...
void ColorPanel::add_controls() {
  ...
  CollapsibleGroup *cg = new CollapsibleGroup(g->x(), 20, g->w(), 20, "LUT");
  ...
  auto bW =
      new Widget<Fl_Button>(g->x() + g->w() - 30, 20, 30, 20, "@fileopen");
  b = bW;
  ...
  gb->begin();
  auto mW = new Widget<Fl_Choice>(g->x() + 100, 21, g->w() - 100, 20, "Order");
  Fl_Choice *m = mW;
  ...
  Fl_Check_Button *c;
  HorSlider *s;
  auto cV = new Widget<Fl_Check_Button>(g->x() + 90, 50, g->w(), 20, "Enabled");
  c = colorOn = cV;
  c->labelsize(12);
  cV->callback([=](auto w) {
  ...
  cV = new Widget<Fl_Check_Button>(g->x() + 90, 50, g->w(), 20, "Invert");
  c = cV;
  ...
  cV = new Widget<Fl_Check_Button>(g->x() + 90, 50, g->w(), 20, "Enabled");
  c = levelsOn = cV;
  c->labelsize(12);
  cV->callback([=](auto w) {
    timeline::DisplayOptions &o = p.ui->uiView->getDisplayOptions(0);
  ...
  cV = new Widget<Fl_Check_Button>(g->x() + 90, 120, g->w(), 20, "Enabled");
  c = softClipOn = cV;
  c->labelsize(12);
  cV->callback([=](auto w) {
    timeline::DisplayOptions &o = p.ui->uiView->getDisplayOptions(0);
