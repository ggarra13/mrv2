...
ComparePanel::ComparePanel(ViewerUI *ui) : _r(new Private), PanelWidget(ui) {
...
void ComparePanel::add_controls() {
  ...
  _r->thumbnailCreator = p.ui->uiTimeWindow->uiTimeline->thumbnailCreator();
  ...
  s = wipeRotation = sV;
  s->tooltip(_(
      "Wipe Rotation.  Use Shift + left mouse button along X to rotate wipe."));
  s->range(0.f, 360.0f);
...
void ComparePanel::redraw() {
  ...
  for (auto &m : _r->map) {
    ...
    if (!found) {
      b->value(0);
      b->redraw();
      if (b->image() && (o.mode == timeline::CompareMode::A || i != Aindex))
        continue;
    }
