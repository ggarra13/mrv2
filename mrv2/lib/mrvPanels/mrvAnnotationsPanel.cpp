...
AnnotationsPanel::AnnotationsPanel(ViewerUI *ui) : PanelWidget(ui) {
...
void AnnotationsPanel::add_controls() {
  ...
  auto cW =
      new Widget<Fl_Choice>(g->x() + 100, Y + 20, g->w() - 100, 20, _("Font"));
  c = cW;
  c->labelsize(12);
...
#else
  const char *kFonts[3] = {"NotoSans-Regular", "NotoSans-Bold",
                           "NotoMono-Regular"};
  ...
  int font = std_any_empty(value) ? FL_HELVETICA : std_any_cast<int>(value);
  if (font > numFonts)
    font = 0;
  c->value(font);
  ...
  d->range(1, 50);
  d->step(1);
  d->tooltip(_("Selects the number of fading frames previous to the frame of "
               "the annotation."));
  ...
  d->range(1, 50);
  d->step(1);
  d->tooltip(_("Selects the number of fading frames following the frame of the "
               "annotation."));
  ...
  auto rV = new Widget<Fl_Radio_Round_Button>(X, 40, 50, 20, _("Current"));
  r = rV;
  ...
  rV->callback(
      [=](auto w) { settingsObject->setValue(kAllFrames, (int)!w->value()); });
