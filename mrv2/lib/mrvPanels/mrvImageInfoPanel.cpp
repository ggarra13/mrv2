...
static bool process_row(int row, Fl_Widget *w, const std::string &match,
                        MatchType type) {
...
ImageInfoPanel::ImageInfoPanel(ViewerUI *ui) : PanelWidget(ui) {
...
void ImageInfoPanel::add_controls() {
  ...
  flex =
      new Fl_Flex(controls->x(), controls->y(), controls->w(), controls->h());
  ...
  m_entry->when(FL_WHEN_CHANGED | FL_WHEN_NOT_CHANGED | FL_WHEN_ENTER_KEY);
...
static void change_first_frame_cb(Fl_Int_Input *w, ImageInfoPanel *info) {
  ...
  range =
      otime::TimeRange::range_from_start_end_time_inclusive(start, end_time);
...
static void change_last_frame_cb(Fl_Int_Input *w, ImageInfoPanel *info) {
  ...
  range =
      otime::TimeRange::range_from_start_end_time_inclusive(start_time, end);
...
void ImageInfoPanel::add_button(const char *name, const char *tooltip,
                                Fl_Callback *callback, Fl_Callback *callback2) {
...
void ImageInfoPanel::add_scale(const char *name, const char *tooltip,
...
void ImageInfoPanel::add_ocio_ics(const char *name, const char *tooltip,
                                  const char *content, const bool editable,
                                  Fl_Callback *callback) {
  if (!editable)
...
void ImageInfoPanel::add_text(const char *name, const char *tooltip,
                              const char *content, const bool editable,
                              const bool active, Fl_Callback *callback) {
  Fl_Color colA = get_title_color();
  Fl_Color colB = get_widget_color();
...
void ImageInfoPanel::add_text(const char *name, const char *tooltip,
                              const std::string &content, const bool editable,
                              const bool active, Fl_Callback *callback) {
...
void ImageInfoPanel::add_enum(const char *name, const char *tooltip,
                              const size_t content, const char *const *options,
                              const size_t num, const bool editable,
                              Fl_Callback *callback) {
  Fl_Color colA = get_title_color();
  Fl_Color colB = get_widget_color();
...
void ImageInfoPanel::add_enum(const char *name, const char *tooltip,
                              const std::string &content, stringArray &options,
                              const bool editable, Fl_Callback *callback) {
  size_t index;
  stringArray::iterator it = std::find(options.begin(), options.end(), content);
  if (it != options.end()) {
...
void ImageInfoPanel::add_int(const char *name, const char *tooltip,
                             const unsigned int content, const bool editable,
                             const bool active, Fl_Callback *callback,
...

  {
    ...
    if (!editable) {
      ...
      mrv::Slider *slider = new mrv::Slider(kMiddle + 60, Y, p->w() - 60, hh);
...
void ImageInfoPanel::add_int64(const char *name, const char *tooltip,
...
void ImageInfoPanel::add_rect(const char *name, const char *tooltip,
  ...
  {
    Fl_Int_Input *widget = new Fl_Int_Input(kMiddle + dw * 4, Y, dw, hh, "W:");
  ...
  {
    Fl_Int_Input *widget = new Fl_Int_Input(kMiddle + dw * 5, Y, dw, hh, "H:");
...
void ImageInfoPanel::add_float(const char *name, const char *tooltip,
...
void ImageInfoPanel::add_bool(const char *name, const char *tooltip,
                              const bool content, const bool editable,
                              Fl_Callback *callback) {
  Fl_Color colA = get_title_color();
  Fl_Color colB = get_widget_color();
...
void ImageInfoPanel::fill_data() {
  ...
  add_time(_("Start Time"), _("Beginning frame of clip"), startTime, false);
  ...
  add_int(_("First Frame"), _("First frame of clip - User selected"),
          ...
          last);
  add_int(_("Last Frame"), _("Last frame of clip - User selected"), (int)last,
          true, true, (Fl_Callback *)change_last_frame_cb, 2, last);

  float fps = player->speed();
  add_float(_("Current Speed"), _("Current Speed (Frames Per Second)"), fps,
  ...
  if (num_video_streams > 0) {
    for (int i = 0; i < num_video_streams; ++i) {
      ...
      add_int(_("Width"), _("Width of clip"), (unsigned)size.w, false);
      add_int(_("Height"), _("Height of clip"), (unsigned)size.h, false);
      ...
      add_text(_("Default Speed"), _("Default Speed in Frames per Second"),
               buf);
