...
SettingsPanel::SettingsPanel(ViewerUI *ui) : PanelWidget(ui) {
  add_group("Settings");
...
void SettingsPanel::add_controls() {
  ...
  auto sV = new Widget<HorSlider>(g->x(), 90, g->w(), 20, _("Read Ahead"));
  s = sV;
  ...
  bg->begin();
  auto mW =
      new Widget<Fl_Choice>(g->x() + 130, 130, g->w() - 130, 20, _("Audio"));
  Fl_Choice *m = mW;
  m->labelsize(12);
  ...
  DBG;
  auto inW = new Widget<Fl_Int_Input>(g->x() + 130, 190, g->w() - 130, 20,
                                      _("Maximum Digits"));
  i = inW;
  i->labelsize(12);
  i->color((Fl_Color)-1733777408);
  i->textcolor(FL_BLACK);
  digits =
      std_any_cast<int>(settingsObject->value("Misc/MaxFileSequenceDigits"));
  ...
  // i->range( 1, 64 );
  digits =
      std_any_cast<int>(settingsObject->value("Performance/VideoRequestCount"));
  ...
  // i->range( 1, 64 );
  digits =
      std_any_cast<int>(settingsObject->value("Performance/AudioRequestCount"));
  text = string::Format("{0}").arg(digits);
  i->value(text.c_str());
  inW->callback([=](auto o) {
    int requests = atoi(o->value());
  ...
  inW->callback([=](auto o) {
    int requests = atoi(o->value());
    settingsObject->setValue("Performance/SequenceThreadCount", requests);
    p.ui->app->_cacheUpdate();
  });

  bg->end();

  auto cV = new Widget<Fl_Check_Button>(g->x() + 90, 370, g->w(), 20,
                                        _("FFmpeg YUV to RGB "
                                          "conversion"));
  c = cV;
  c->labelsize(12);
  ...
  i->textcolor(FL_BLACK);
  digits =
      std_any_cast<int>(settingsObject->value("Performance/FFmpegThreadCount"));
  text = string::Format("{0}").arg(digits);
  i->value(text.c_str());
  inW->callback([=](auto o) {
    int requests = atoi(o->value());
  ...
  auto bW =
      new Widget<Fl_Button>(g->x(), g->y(), g->w(), 20, _("Default Settings"));
  b = bW;
  b->box(FL_UP_BOX);
  bW->callback([=](auto o) {
