...
void PanelGroup::dock_grp(void *v) { // dock CB
  PanelGroup *gp = (PanelGroup *)v;
...
void PanelGroup::undock_grp(void *v) { // undock CB
  PanelGroup *gp = (PanelGroup *)v;
  DockGroup *dock = gp->get_dock();

  if (gp->docked()) { // undock the group into its own non-modal tool window
    int W = gp->w();
    int H = gp->h();
...
void PanelGroup::cb_dismiss(Fl_Button *, void *v) {
  ...
  if (gp->docked()) { // remove the group from the dock
    dock->remove(gp);
    gp->docked(false);
    dock->redraw(); // update the dock, to show the group has gone...
    Fl::delete_widget(gp);
  } else { // remove the group from the floating window,
...
PanelGroup::PanelGroup(DockGroup *dk, int floater, int x, int y, int w, int h,
                       const char *lbl)
...
void PanelGroup::create_floating(DockGroup *dk, int full, int x, int y, int w,
