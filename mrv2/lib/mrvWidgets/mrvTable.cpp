...
void Table::draw_cell(TableContext context, int ROW, int COL, int X, int Y,
                      int W, int H) {
  char s[40];
  switch (context) {
...
void Table::layout() {
  ...
  find_cell(CONTEXT_TABLE, R, C, X, Y, W, H);
  bot_y2 = Y + H;
  // Determine overall height of table (diff between top_y1 and bot_y2 plus
  // col_header)
  int dh =
      bot_y2 - top_y1 + col_header_height() + 4; // +4: probably border width
  resize(x(), y(), w(), dh);
}
