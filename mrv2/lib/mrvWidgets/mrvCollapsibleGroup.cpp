...
void CollapsibleGroup::layout() {
  ...
  //
  _button->resize(
      x() + GROUP_MARGIN,       // x always inset (leaves room for ROUND box)
  ...
  _contents->resize(x() + GROUP_MARGIN, // x always same as button
                    y() + _button->h() +
                        GROUP_MARGIN,         // y always "below button"
                    w() - (GROUP_MARGIN * 2), // width tracks group's w()
  ...
  // _contents->w(), _contents->h()); printf("\n");
}
...
// CTOR
CollapsibleGroup::CollapsibleGroup(const int x, const int y, const int w,
                                   const int h, const char *l)
    : Fl_Group(x, y, w, h, l) {
  box(FL_ROUNDED_BOX); // get fancy later, after we debug. It's harder to see
                       // problems with round box. -erco
  ...
  //  Button
  _button =
      new Fl_Button(x + GROUP_MARGIN, // margin leaves room for FL_ROUND_BOX
  ...
  _button->box(
      FL_FLAT_BOX); // commented out for easier debugging. Revert when needed
  _button->callback((Fl_Callback *)toggle_tab_cb, this);

  _contents =
      new Pack(_button->x(),                    // lines up with button on x
               y + _button->y() + _button->h(), // just below button
               w - (GROUP_MARGIN * 2), // width same as group within margin
               10);                    // changes when child add()ed

  // end() _contents; we dont' want it to begin() sucking up child widgets on
  // return
  _contents->end();
  Fl_Group::end();
