...
void copy_color_cb(void *, mrv::Browser *w) {
  ...
  while ((start = line.find('@', 0)) != std::string::npos) {
    ...
    if (start > 0)
      line =
          (line.substr(0, start - 1) + line.substr(last + 1, line.size() - 1));
    else
  ...
  std::string copy = " ";
  last = 0;
  while ((start = line.find_first_not_of("\t ", last)) != std::string::npos) {
...
public:
  ColorBrowser(int X, int Y, int W, int H, const char *L = 0)
      : mrv::Browser(X, Y, W, H, L), _value(-1) {}
  ...
  int mousePush(int X, int Y) {
    ...
    menu.add(_("Copy/Color"), FL_COMMAND + 'C', (Fl_Callback *)copy_color_cb,
             (void *)this, 0);
...
void ColorInfo::update(const area::Info &info) {
  if (info.box.min.x == info.box.max.x || info.box.min.y == info.box.max.y) {
    area->copy_label("");
    area->redraw();
    browser->clear();
    browser->redraw();
  ...
  Fl_Color col;
  {
    ...
    if (r <= 0.01f && g <= 0.01f && b <= 0.01f)
      ...
      col = fl_rgb_color((uchar)(r * 255), (uchar)(g * 255), (uchar)(b * 255));
    }
  }
  ...
  text << "@b\t" << std::fixed << std::setw(7) << std::setprecision(2) << kR
       << "R"
       << "\t" << kG << "G"
       << "\t" << kB << "B"
       << "\t" << kA << "A"
  ...
  text << _("Range") << ":\t@c" << info.rgba.diff.r << "\t@c"
       ...
       << info.rgba.diff.a << "\n"
       << "@b" << _("Mean") << ":\t@c" << kR << info.rgba.mean.r << "\t@c" << kG
       << info.rgba.mean.g << "\t@c" << kB << info.rgba.mean.b << "\t@c" << kA
       << info.rgba.mean.a << "\n"
       << "\n"
       << "@b\t";
  ...
  text << "\n"
       << _("Maximum") << ":\t@c" << info.hsv.max.r << "\t@c" << info.hsv.max.g
       << "\t@c" << info.hsv.max.b << "\t@c" << info.hsv.max.a << "\n"
       << _("Minimum") << ":\t@c" << info.hsv.min.r << "\t@c" << info.hsv.min.g
       << "\t@c" << info.hsv.min.b << "\t@c" << info.hsv.min.a << "\n"
       << _("Range") << ":\t@c" << info.hsv.diff.r << "\t@c" << info.hsv.diff.g
       << "\t@c" << info.hsv.diff.b << "\t@c" << info.hsv.diff.a << "\n"
       << "@b" << _("Mean") << ":\t@c" << kH << info.hsv.mean.r << "\t@c" << kS
       << info.hsv.mean.g << "\t@c" << kV << info.hsv.mean.b << "\t@c" << kL
       << info.hsv.mean.a << "\n";
