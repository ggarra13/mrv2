...
struct FormatInfo {
  ...
  FormatInfo(bool dec, bool enc, bool blo, const char *n, const char *mod,
             const char *desc)
      : encode(enc), decode(dec), blob(blo), name(n), module(mod),
        description(desc) {}
...
std::string about_message() {
  ...
  o << "mrv2 " << kArch << " bits - v" << kVersion << " " << kBuild << endl
#ifdef __GLIBCXX__
  ...
  o << endl
    ...
    << endl
    << "Boost v" << boost_major << "." << boost_minor << "." << boost_teeny
    << endl
...
#ifdef _WIN32
void memory_information(uint64_t &totalVirtualMem, uint64_t &virtualMemUsed,
...
std::string gpu_information(ViewerUI *ui) {
  ...
  o << "HW Stereo:\t" << (ui->uiView->can_do(FL_STEREO) ? "Yes" : "No") << endl
    << "HW Overlay:\t" << (ui->uiView->can_do_overlay() ? "Yes" : "No") << endl;
