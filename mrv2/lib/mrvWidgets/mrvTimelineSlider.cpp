...
TimelineSlider::TimelineSlider(int x, int y, int w, int h, char *l)
    : Slider(x, y, w, h, l), _p(new Private) {
  type(TICK_ABOVE);
  slider_type(kNORMAL);
}
...
void TimelineSlider::hideThumbnail_cb(TimelineSlider *t) { t->hideThumbnail(); }
...
int TimelineSlider::handle(int e) {
  ...
  if (e == FL_ENTER) {
    ...
    if (!Fl::has_timeout((Fl_Timeout_Handler)hideThumbnail_cb, this)) {
      Fl::add_timeout(0.005, (Fl_Timeout_Handler)hideThumbnail_cb, this);
    }
    return 1;
...
otime::RationalTime TimelineSlider::_posToTime(int value) const noexcept {
  ...
  if (p.timelinePlayer && p.timelinePlayer->timelinePlayer()) {
    ...
    out = otime::RationalTime(
        floor(math::clamp(value, 0, width) / static_cast<double>(width) *
                  duration.value() +
  ...
  if (id == p.thumbnailRequestId) {
    for (const auto &i : thumbnails) {
      ...
      if (image) {
        ...
        p.images.erase(std::remove(p.images.begin(), p.images.end(), image),
                       p.images.end());
