...
/* If called with maxbuf==0, use an internally allocated buffer and enlarge it
 as needed. Otherwise, use buf as buffer but don't go beyond its length of
 maxbuf.
*/
static const char *expand_text_(const char *from, char *&buf, int maxbuf,
  ...
  for (;; p++) {
    ...
    if (c == '\t') {
      for (c = fl_utf_nb_char((uchar *)buf, (int)(o - buf)) % 8; c < 8 && o < e;
           c++)
        *o++ = ' ';
    } else if (c < ' ' || c == 127) { // ^X
      *o++ = '^';
...
int MultilineInput::textsize() const { return Fl_Multiline_Input::textsize(); }
