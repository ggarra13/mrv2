...
ProgressReport::ProgressReport(Fl_Window *main, int64_t start, int64_t end)
    : _frame(start), _start(start), _end(end), _time(0) {
#if 1
  Fl_Group::current(0);
  ...
  w = new Fl_Window(main->w() / 2 - 320, main->h() / 2 - 120 / 2, 640, 120);
#endif
  ...
  char title[1024];
  snprintf(title, 1024, _("Saving Movie %" PRId64 " - %" PRId64), start, end);
...
void ProgressReport::to_hour_min_sec(const double t, int &hour, int &min,
                                     int &sec) {
