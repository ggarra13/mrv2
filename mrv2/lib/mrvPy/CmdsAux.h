...
namespace
{
    ...
    inline std::ostream&
    operator<<(std::ostream& o, const mrv::FilesModelItem& a)
    {
        o << "<mrv2.FileMedia path=" << a.path.get()
          << " audioPath=" << a.audioPath.get() << " timeRange=" << a.timeRange
          << " speed=" << a.speed << " playback=" << a.playback
          << " loop=" << a.loop << " currentTime=" << a.currentTime
          << " inOutRange=" << a.inOutRange << " videoLayer=" << a.videoLayer
          << " volume=" << a.volume << " mute=" << a.mute
