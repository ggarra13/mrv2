//
// Flu_Scroll widget for the Fast Light Tool Kit (FLTK).
//
...
void Flu_Scroll::clear()
{
    ...
    // and deletion. Finally they are added to Flu_Scroll's group again. This
...
/** Ensure the scrollbars are the last children.
  ...
  When Flu_Scroll is instantiated the first child of the Fl_Group is the
...
*/
void Flu_Scroll::fix_scrollbar_order()
{
    Fl_Widget** a = (Fl_Widget**)array();
    if (children() > 1 &&
        (a[children() - 2] != &scrollbar || a[children() - 1] != &hscrollbar))
    {
        int i, j;
...
*/
int Flu_Scroll::on_insert(Fl_Widget* candidate, int index)
{
    if (children() > 1 && index > children() - 2 && candidate != &scrollbar &&
        candidate != &hscrollbar)
    {
        index = children() - 2;
    }
    return index;
 ...
 */
int Flu_Scroll::on_move(int old_index, int new_index)
{
...
/**
  ...
  \since FLTK 1.4.0
*/
int Flu_Scroll::delete_child(int index)
{
...
//
void Flu_Scroll::draw_clip(void* v, int X, int Y, int W, int H)
{
    fl_push_clip(X, Y, W, H);
    Flu_Scroll* s = (Flu_Scroll*)v;
    ...
    case _FL_PLASTIC_DOWN_FRAME:
        ...
        {
            Fl::scheme_bg_->draw(
                X - (X % ((Fl_Tiled_Image*)Fl::scheme_bg_)->image()->w()),
...
/**
  ...
  \see bbox()
*/
void Flu_Scroll::recalc_scrollbars(ScrollInfo& si) const
{
    ...
    // See if children would fit if we had no scrollbars...
    {
        ...
        si.scrollsize =
            scrollbar_size_ ? scrollbar_size_ : Fl::scrollbar_size();
        si.vneeded = 0;
        si.hneeded = 0;
        if (type() & VERTICAL)
        {
        ...
        if (type() & HORIZONTAL)
        {
            if ((type() & ALWAYS_ON) || si.child.l < X || si.child.r > X + W)
            {
                ...
                {
                    if ((type() & ALWAYS_ON) || si.child.t < Y ||
                        si.child.b > Y + H)
                    {
                        si.vneeded = 1;
                        W -= si.scrollsize;
    ...
    //  printf("DEBUG     innerbox.x, si.innerbox.y,
    //  si.innerbox.w,si.innerbox.h); printf("DEBUG   innerchild.xywh: %d %d %d
    //  %d\n", si.innerchild.x, si.innerchild.y, si.innerchild.w,
    //  si.innerchild.h); printf("DEBUG        child lrbt: %d %d %d %d\n",
    //  si.child.l, si.child.r, si.child.b, si.child.t); printf("DEBUG hscroll
    //  xywh: %d %d %d %d\n", si.hscroll.x, si.hscroll.y, si.hscroll.w,
    //  si.hscroll.h); printf("DEBUG      vscroll xywh: %d %d %d %d\n",
    //  si.vscroll.x, si.vscroll.y, si.vscroll.w, si.vscroll.h); printf("DEBUG
    //  horz scroll vals: %d %d %d %d\n", si.hscroll.pos, si.hscroll.size,
    //  si.hscroll.first, si.hscroll.total); printf("DEBUG  vert scroll vals: %d
    //  %d %d %d\n", si.vscroll.pos, si.vscroll.size, si.vscroll.first,
    //  si.vscroll.total); printf("DEBUG \n");
}
...
*/
void Flu_Scroll::bbox(int& X, int& Y, int& W, int& H) const
{
    ScrollInfo si;
...
void Flu_Scroll::draw()
{
    ...
    else
    {
        if (d & FL_DAMAGE_SCROLL)
        {
            // scroll the contents:
            fl_scroll(
                X, Y, W, H, oldx - xposition_, oldy - yposition_, draw_clip,
                this);
            ...
            if (L > X)
                draw_clip(this, X, Y, L - X, H);
            if (R < (X + W))
                draw_clip(this, R, Y, X + W - R, H);
            if (T > Y)
                draw_clip(this, X, Y, W, T - Y);
            if (B < (Y + H))
                draw_clip(this, X, B, W, Y + H - B);
        }
    ...
    // Calculate where scrollbars should go, and draw them
    {
        ...
        else if (!si.vneeded && scrollbar.visible())
        {
            ...
            draw_clip(
                this, si.vscroll.x, si.vscroll.y, si.vscroll.w, si.vscroll.h);
            d = FL_DAMAGE_ALL;
        }
        ...
        else if (!si.hneeded && hscrollbar.visible())
        {
            ...
            draw_clip(
                this, si.hscroll.x, si.hscroll.y, si.hscroll.w, si.hscroll.h);
            d = FL_DAMAGE_ALL;
        }
        else if (
            hscrollbar.h() != si.scrollsize || scrollbar.w() != si.scrollsize)
        {
            // scrollsize changed
            d = FL_DAMAGE_ALL;
        }
        ...
        hscrollbar.value(
            si.hscroll.pos, si.hscroll.size, si.hscroll.first,
            si.hscroll.total);
    }
    ...
    if (d & FL_DAMAGE_ALL)
    {
        ...
        if (scrollbar.visible() && hscrollbar.visible())
        {
            ...
            fl_color(color());
            fl_rectf(
                scrollbar.x(), hscrollbar.y(), scrollbar.w(), hscrollbar.h());
        }
    }
    else
    {
...
/**
  Resizes the Flu_Scroll widget and moves its children if necessary.

  The Flu_Scroll widget first resizes itself, and then it moves all its
  children if (and only if) the Flu_Scroll widget has been moved. The
  children are moved by the same amount as the Flu_Scroll widget has been
  moved, hence all children keep their relative positions.

  \note Flu_Scroll::resize() does \b not call Fl_Group::resize(), and
  child widgets are \b not resized.

  Since children of an Flu_Scroll are not resized, the resizable() widget
  is ignored (if it is set).

  The scrollbars are moved to their proper positions, as given by
  Flu_Scroll::scrollbar.align(), and switched on or off as necessary.

  \note Due to current (FLTK 1.3.x) implementation constraints some of this
  may effectively be postponed until the Flu_Scroll is drawn the next time.
  This may change in a future release.

  \sa Fl_Group::resizable()
  \sa Fl_Widget::resize(int,int,int,int)
*/
void Flu_Scroll::resize(int X, int Y, int W, int H)
{
    int dx = X - x(), dy = Y - y();
    int dw = W - w(), dh = H - h();
    ...
    if (dw == 0 && dh == 0)
    {
        ...
        scrollbar.position(
            al ? X : X + W - scrollbar.w(),
            (at && pad) ? Y + hscrollbar.h() : Y);
        hscrollbar.position(
            (al && pad) ? X + scrollbar.w() : X,
            at ? Y : Y + H - hscrollbar.h());
    }
    else
    {
        // FIXME recalculation of scrollbars needs to be moved out of "draw()"
        // (STR #1895)
...
/**  Moves the contents of the scroll group to a new position.
  ...
  This is like moving the scrollbars of the Flu_Scroll around. For instance:
  \code
    Flu_Scroll scroll (10,10,200,200);
  ...
  relative to the origin of the Flu_Scroll (10,10), i.e. Fl_Box b3 will
  be visible in the top left corner of the scroll area.
*/
void Flu_Scroll::scroll_to(int X, int Y)
{
    int dx = xposition_ - X;
    int dy = yposition_ - Y;
    if (!dx && !dy)
...
void Flu_Scroll::hscrollbar_cb(Fl_Widget* o, void*)
{
    Flu_Scroll* s = (Flu_Scroll*)(o->parent());
    s->scroll_to(int(((Fl_Scrollbar*)o)->value()), s->yposition());
}

void Flu_Scroll::scrollbar_cb(Fl_Widget* o, void*)
{
    Flu_Scroll* s = (Flu_Scroll*)(o->parent());
...
/**
  Creates a new Flu_Scroll widget using the given position,
  size, and label string. The default boxtype is FL_NO_BOX.

  The destructor <I>also deletes all the children</I>. This allows a
  whole tree to be deleted at once, without having to keep a pointer to
  all the children in the user code. A kludge has been done so the
  Flu_Scroll and all of its children can be automatic (local)
...
Flu_Scroll::Flu_Scroll(int X, int Y, int W, int H,
    ...
    scrollbar(
        ...
        H - Fl::scrollbar_size()),
    hscrollbar(
        X, Y + H - Fl::scrollbar_size(), W - Fl::scrollbar_size(),
        Fl::scrollbar_size())
{
    type(BOTH);
    chooser_ = c;
