...
std::string Flu_File_Chooser::contextMenuTxt[3] = {"New Folder", "Rename",
                                                   "Delete"};
...
std::string Flu_File_Chooser::createFolderErrTxt =
    "Could not create directory '%s'. You may not have permission to perform "
    "this operation.";
...
static std::string flu_get_special_folder(int csidl) {
  ...
  if (RegOpenKeyEx(HKEY_CURRENT_USER,
                   "Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\She"
                   "ll Folders",
...
void Flu_File_Chooser::add_type(const char *extensions,
                                const char *short_description, Fl_Image *icon) {
  std::string ext;
  if (extensions)
    ext = extensions;
  else
...
Flu_File_Chooser::Flu_File_Chooser(const char *pathname, const char *pat,
                                   int type, const char *title,
                                   const bool compact)
    : Fl_Double_Window(900, 600, title), _p(new Private), _compact(compact),
      filename(70, h() - 60, w() - 70 - 85 - 10, 25, "", this),
      ok(w() - 90, h() - 60, 85, 25), cancel(w() - 90, h() - 30, 85, 25),
      wingrp(new Fl_Group(0, 0, 900, 600)), entryPopup(0, 0, 0, 0) {
  ...
  // name in the field we do this because the actual name may vary from country
  // to country
  {
  ...
  g = new Fl_Group(110, 40, w() - 110,
                   30); // group enclosing all the buttons at top
  ...
  fileGroup = new Fl_Group(110, 70, w() - 120 - 5, h() - 80 - 40 - 15);
  {
    ...
    filelist = new FileList(fileGroup->x() + 2, fileGroup->y() + 2,
                            fileGroup->w() - 4, fileGroup->h() - 4, this);
    ...
    {
      fileDetailsGroup = new Fl_Group(fileGroup->x() + 2, fileGroup->y() + 2,
                                      fileGroup->w() - 4, fileGroup->h() - 4);

      {
        ...
        filescroll->type(Fl_Scroll::BOTH);
        {
          filedetails = new FileDetails(
              filescroll->x() + 2, filescroll->y() + 2, filescroll->w() - 4,
              filescroll->h() - 20 - 4, this);
          filedetails->end();
        }
        filescroll->end();
        {
          filecolumns = new FileColumns(fileGroup->x() + 2, fileGroup->y() + 2,
  ...
  filePattern = new Flu_Combo_List(70, h() - 30, w() - 70 - 85 - 10, 25,
...
void Flu_File_Chooser::pattern(const char *p) {
  ...
  // like this: "Description (patterns)" or just "patterns" where patterns is of
  // the form *.xxx or *.{xxx,yyy,zzz}}
...
int Flu_File_Chooser::handle(int event) {
  ...
  if (Fl_Double_Window::handle(event))
    ...
    cancel.do_callback();
    return 1;
  } else if (event == FL_KEYDOWN && Fl::event_key('a') &&
...
void Flu_File_Chooser::newFolderCB() {
  ...
  // create a new entry with the name of the new folder. add to either the list
  // or the details
  Entry *entry =
...
void Flu_File_Chooser::trashCB(bool recycle) {
  ...
  if (selected) {
    if (selected == 1) {
      if (recycle) {
        if (!mrv::fl_choice("Really send '%s' to the Recycle Bin?", "No", "Yes",
                            0, first))
          return;
      } else {
      ...
      if (recycle) {
        if (!mrv::fl_choice("Really send these %d files to the Recycle Bin?",
                            "No", "Yes", 0, selected))
          return;
      } else {
        if (!mrv::fl_choice("Really delete these %d files?", "No", "Yes", 0,
                            selected))
          return;
      }
    }
    ...
    for (i = 0; i < g->children(); i++) {
      if (((Entry *)g->child(i))->selected) {
        ...
        if (((Entry *)g->child(i))->type == ENTRY_DIR) {
          // if we are recycling in windows, then the recursive part happens
          // automatically
#ifdef _WIN32
          if (!recycle)
#endif
          {
            ...
            Flu_Label *label =
                new Flu_Label(30, 30, 150, 30, "Preparing to delete...");
            win->end();
            win->show();
            Fl::check();
...
#ifdef _WIN32
        // this moves files to the recycle bin, depending on the value of
        // 'recycle'
        {
...
void Flu_File_Chooser::updateLocationQJ() {
  ...
  while (slash) {
    ...
    Fl_Button *b =
        new Fl_Button(locationQuickJump->x() + offset, locationQuickJump->y(),
                      w, locationQuickJump->h(), av_strdup(blank));
  ...
  Fl_Button *b =
      new Fl_Button(locationQuickJump->x() + offset, locationQuickJump->y(), 1,
...
void Flu_File_Chooser::favoritesCB() { cd(FAVORITES_UNIQUE_STRING); }

void Flu_File_Chooser::myComputerCB() { cd("/"); }

void Flu_File_Chooser::documentsCB() { cd(userDocs.c_str()); }

Flu_File_Chooser::FileInput::FileInput(int x, int y, int w, int h,
                                       const char *l, Flu_File_Chooser *c)
    : Fl_Input(x, y, w, h, l) {
  chooser = c;
}
      ...
      if (!chooser->lastSelected) {
        if (chooser->getEntryGroup()->children()) {
          Flu_File_Chooser::Entry *e =
              (Flu_File_Chooser::Entry *)chooser->getEntryGroup()->child(0);
          e->selected = true;
...
Flu_File_Chooser::CBTile::CBTile(int x, int y, int w, int h,
                                 Flu_File_Chooser *c)
    : Fl_Tile(x, y, w, h) {
  chooser = c;
}
...
Flu_File_Chooser::FileColumns::FileColumns(int x, int y, int w, int h,
                                           Flu_File_Chooser *c)
    : Fl_Tile(x, y, w, h) {
  chooser = c;
  ...
  c->detailNameBtn->callback(Flu_File_Chooser::_sortCB, c);
  {
    ...
    c->detailTypeBtn->callback(Flu_File_Chooser::_sortCB, c);
    {
      ...
      Fl_Box *box =
          new Fl_Box(tile->x() + 50, tile->y(), tile->w() - 100, tile->h());
      tile->add(*box);
      c->detailSizeBtn =
      ...
      c->detailDateBtn =
          new Flu_Button(x + W1 + W2 + W3, y, W4, h, _(detailTxt[2].c_str()));
...
void Flu_File_Chooser::FileColumns::resize(int x, int y, int w, int h) {
  ...
  chooser->filescroll->resize(x, y + 20, w,
                              chooser->fileDetailsGroup->h() - 20);
  chooser->updateEntrySizes();
  chooser->redraw();
}
...
void Flu_File_Chooser::_qSort(int how, bool caseSort, Fl_Widget **array,
                              int low, int high) {
...
void Flu_File_Chooser::FileList::sort(int n) {
  ...
  Flu_File_Chooser::_qSort(chooser->sortMethod, chooser->caseSort,
                           (Fl_Widget **)array(), 0, numDirs - 1);
  Flu_File_Chooser::_qSort(chooser->sortMethod, chooser->caseSort,
                           (Fl_Widget **)array(), numDirs, children() - 1);
  chooser->redraw();
}
...
int Flu_File_Chooser::FileList::handle(int event) {
  ...
  if (event == FL_PUSH && !Fl::event_key(FL_SHIFT) && !Fl::event_key(FL_CTRL)) {
    ...
    chooser->filename.insert_position(chooser->filename.size(),
    ...
    if (e) {
      ...
      case FL_Up:
        ...
        if (!e && children())
          e = (Flu_File_Chooser::Entry *)child(0);
        break;
      case FL_Down:
        ...
        break;
      case FL_Left:
        e = (Flu_File_Chooser::Entry *)left(e);
        break;
      case FL_Right:
        e = (Flu_File_Chooser::Entry *)right(e);
        break;
      case FL_Home:
        if (children())
          e = (Flu_File_Chooser::Entry *)child(0);
...
int Flu_File_Chooser::Entry::handle(int event) {
  if (editMode) {
    // if user hits 'Escape' while in edit mode, restore the original name and
    // turn off edit mode
  ...
  if (event == FL_PUSH) {
    ...
    if (chooser->selectionType & MULTI) {
      if (Fl::event_state(FL_CTRL)) {
        ...
        if (chooser->lastSelected == nullptr) {
          ...
          if (lastindex >= 0 && thisindex >= 0) {
            // loop from this item to the last item, toggling each item except
            // the last
            int inc;
    ...
    // don't put the filename into the box if we are a directory but we are not
    // choosing directories or if we are in SAVING mode
    if ((chooser->selectionType & Flu_File_Chooser::DIRECTORY) ||
        (chooser->selectionType & Flu_File_Chooser::STDFILE) ||
        type == ENTRY_FILE || type == ENTRY_SEQUENCE)
      chooser->filename.value(filename.c_str());
    ...
    if (chooser->selectionType & MULTI) {
      ...
      if (chooser->lastSelected != nullptr) {
        ...
        if (lastindex >= 0 && thisindex >= 0) {
          // loop from this item to the last item, toggling each item except the
          // last
          int inc;
          if (thisindex > lastindex)
            inc = -1;
...
int Flu_File_Chooser::popupContextMenu(Entry *entry) {
  ...
  case ENTRY_NONE: // right click on nothing
    entryPopup.add(_(contextMenuTxt[0].c_str()), 0, 0,
...
void Flu_File_Chooser::Entry::draw() {
  ...
  if (details) {
    if (shortDescription[0] != '\0')
      fl_draw(shortDescription.c_str(), X, y(), typeW - 4, h(),
              Fl_Align(FL_ALIGN_LEFT | FL_ALIGN_CLIP));
    else
      fl_draw(description.c_str(), X, y(), typeW - 4, h(),
              Fl_Align(FL_ALIGN_LEFT | FL_ALIGN_CLIP));

    X += typeW;

    fl_draw(filesize.c_str(), X, y(), sizeW - 4, h(),
            Fl_Align(FL_ALIGN_RIGHT | FL_ALIGN_CLIP));

    X += sizeW + 4;

    fl_draw(date.c_str(), X, y(), dateW - 4, h(),
            Fl_Align(FL_ALIGN_LEFT | FL_ALIGN_CLIP));
  }
}
...
void Flu_File_Chooser::updateEntrySizes() {
  ...
  // column
  filedetails->resize(filedetails->x(), filedetails->y(), filescroll->w(),
                      filedetails->h());
  for (i = 0; i < filedetails->children(); ++i)
...
#endif
      if (((Entry *)g->child(i))->selected ||
          (currentDir + ((Entry *)g->child(i))->filename) == filename.value()) {
...
std::string Flu_File_Chooser::formatDate(const char *d) {
  ...
  sscanf(d, "%s %s %d %d:%d:%d %d", dummy, MM, &day, &hour, &minute, &second,
         &year);
  ...
  snprintf(dummy, 64, "%d/%d/%02d %d:%02d %s", month, day, year, hour, minute,
           pm ? "PM" : "AM");
...
void Flu_File_Chooser::locationCB(const char *path) {
  ...
  if (p == favs)
    ...
    desktopCB();
  // if the path leads off with "/Desktop/My Computer", then strip that part off
  // and cd to the remaining
  else {
    std::string s = mycomp;
      ...
      if (mask & 1) {
        ...
        if (refreshDrives) {
          ...
          if (type != DRIVE_REMOVABLE && type != DRIVE_REMOTE)
            GetVolumeInformation(windrive, volumeName, 1024, nullptr, nullptr,
                                 nullptr, nullptr, 0);
...
void Flu_File_Chooser::cd(const char *path) {
  ...
  if (streq(path, FAVORITES_UNIQUE_STRING)) {
    ...
    for (int i = 1; i <= favoritesList->size(); i++) {
      ...
      entry = new Entry(favoritesList->text(i), ENTRY_FAVORITE,
  ...
  // check for parent directory
    ...
    // if we are viewing the favorites and want to go back a directory, go to
    ...
    // currentDir = userHome;
    // if we are viewing "My Computer" and want to go back a directory, go to
    // the desktop
    if (currentDir == "/") {
  ...
  // check for desktop. if so, add My Computer and My Documents
    ...
    if (listMode)
      ...
      filedetails->begin();
    entry = new Entry(myDocumentsTxt.c_str(), ENTRY_MYDOCUMENTS,
                      fileDetailsBtn->value(), this);
    entry->updateSize();
    entry->updateIcon();
    entry = new Entry(myComputerTxt.c_str(), ENTRY_MYCOMPUTER,
                      fileDetailsBtn->value(), this);
    entry->updateSize();
    entry->updateIcon();
    if (listMode)
  ...
  if (num < 0) {
    // LOG_ERROR( _("Listing of directory \"") << pathbase << _("\" failed with
  ...
  if (num > 0) {
    ...
    for (i = 0; i < num; i++) {
      ...
      // only directories?
      if ((selectionType & DIRECTORY) && !isDir && !(selectionType & STDFILE) &&
      ...
      if (isDir) {
        ...
        if (compact_files()) {
          ...
          if (mrv::is_valid_movie(tmp.c_str()) ||
              ...
              mrv::is_valid_subtitle(tmp.c_str()) || tmp == _(".ocio") ||
              tmp == _(".prefs"))
            is_sequence = false;
        } else {
          is_sequence = false;
    ...
    //
    {
      ...
      for (; i != e; ++i) {
        entry =
            new Entry((*i).c_str(), ENTRY_DIR, fileDetailsBtn->value(), this);
        if (!entry)
          continue;
    ...
    //
    {
      ...
      for (; i != e; ++i) {
        entry = new Entry((*i).root.c_str(), ENTRY_SEQUENCE,
                          fileDetailsBtn->value(), this);
        entry->isize = 1 + (atoi((*i).ext.c_str()) - atoi((*i).number.c_str()));
...

    {
      ...
      for (; i != e; ++i) {
        entry =
            new Entry((*i).c_str(), ENTRY_FILE, fileDetailsBtn->value(), this);
...
static const char *
_flu_file_chooser(const std::shared_ptr<tl::system::Context> &context,
                  const char *message, const char *pattern,
                  const char *filename, int type, FluStringVector &filelist,
...
size_t
flu_multi_file_chooser(const std::shared_ptr<tl::system::Context> &context,
                       const char *message, const char *pattern,
                       const char *filename, FluStringVector &filelist,
                       const bool compact_files) {
  _flu_file_chooser(context, message, pattern, filename,
                    Flu_File_Chooser::MULTI, filelist, compact_files);
  return filelist.size();
}

const char *
flu_file_chooser(const std::shared_ptr<tl::system::Context> &context,
                 const char *message, const char *pattern, const char *filename,
                 const bool compact_files) {
  FluStringVector filelist;
  return _flu_file_chooser(context, message, pattern, filename,
                           Flu_File_Chooser::SINGLE, filelist, compact_files);
}

const char *
flu_save_chooser(const std::shared_ptr<tl::system::Context> &context,
                 const char *message, const char *pattern, const char *filename,
                 const bool compact_files) {
  FluStringVector filelist;
  return _flu_file_chooser(context, message, pattern, filename,
                           Flu_File_Chooser::SINGLE | Flu_File_Chooser::SAVING,
...
const char *flu_dir_chooser(const std::shared_ptr<tl::system::Context> &context,
  ...
  return _flu_file_chooser(context, message, "*", filename,
                           Flu_File_Chooser::DIRECTORY, filelist);
}
...
const char *flu_dir_chooser(const std::shared_ptr<tl::system::Context> &context,
  ...
  if (showFiles)
    return _flu_file_chooser(context, message, "*", filename,
                             Flu_File_Chooser::DIRECTORY |
                                 Flu_File_Chooser::DEACTIVATE_FILES,
                             filelist);
  else
...
const char *
flu_file_and_dir_chooser(const std::shared_ptr<tl::system::Context> &context,
                         const char *message, const char *filename) {
  FluStringVector filelist;
  return _flu_file_chooser(context, message, "*", filename,
                           Flu_File_Chooser::STDFILE, filelist);
}
