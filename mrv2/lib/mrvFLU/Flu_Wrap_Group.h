...
public:
    ...
    //! \return the widget that is visibly above \b w in the group, or \c NULL
    //! if no such widget exists
    Fl_Widget* above(Fl_Widget* w);

    //! \return the widget that is visibly below \b w in the group, or \c NULL
    //! if no such widget exists
    ...
    //! \return the widget that is logically after \b w in the groups order, or
    //! \c NULL if no such widget exists
    ...
    //! Set the wrap type. Must be either \c FL_VERTICAL (children wrap
    //! according to the width, with a vertical scrollbar) or \c FL_HORIZONTAL
    //! (children wrap according to the height, with a horizontal scrollbar).
    //! Default is \c FL_HORIZONTAL
    void type(int t);
     ...
     * These are strictly for convenience. Only the most commonly called
     * functions have been re-implemented. You can also explicitly access the
     * group object for more control.
     */
    //@{
