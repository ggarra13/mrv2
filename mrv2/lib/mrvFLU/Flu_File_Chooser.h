...
flu_save_chooser(const std::shared_ptr<tl::system::Context> &context,
                 ...
                 const bool compact_files = true);
FLU_EXPORT const char *
flu_dir_chooser(const std::shared_ptr<tl::system::Context> &context,
...
public:
  //! strings to be set by a programmer to the correct phrase or name for their
  //! language
  ...
  /*! Simply derive from this class and overload Fl_Group's methods to create a
    widget able to preview whatever file type you want. Register it with
    Flu_File_Chooser::add_preview_handler() When a file is previewed, all
    registered handlers are visited until the preview() virtual function for one
    of them returns nonzero. When preview() is called, the absolute path of the
    file is passed in, and the widget should determine whether it can preview
    the file and update itself accordingly. If it can preview the file, it
    should return nonzero, else it should return zero.
   */
  //! File entry type
  enum {
  ...
  enum {
    ...
    DIRECTORY = 4, /*!< Choose directories (choosing files is implicit if this
                      bit is clear) */
    DEACTIVATE_FILES = 8, /*!< When choosing directories, also show the files in
                             a deactivated state */
    SAVING = 16, /*!< When choosing files, whether to keep the current filename
                    always in the input area */
  ...
  //! Constructor opening a file chooser with title \b title visiting directory
  //! \b path with files filtered according to \b pattern. \b type is a logical
  //! OR of Flu_File_Chooser::SINGLE, Flu_File_Chooser::MULTI, and
  ...
  /*! \param type is the type of entry to handle (i.e. a logical OR of \c
    ENTRY_NONE, \c ENTRY_DIR, \c ENTRY_FILE, \c ENTRY_FAVORITE, \c ENTRY_DRIVE,
    \c ENTRY_MYDOCUMENTS, \c ENTRY_MYCOMPUTER). To add a "nothing" handler (when
    the user right-clicks on nothing), use ENTRY_NONE \param ext is the
    extension of the file that will cause this handler to be added to the popup
    menu \param name is the name that will appear in the popup menu for this
    handler
   */
  ...
  /*! \param extensions is a space- or comma-delimited list of file extensions,
    or \c NULL for directories. e.g. "zip,tgz,rar" \param short_description is a
    short description (!) of the file type. e.g. "Compressed Archive" \param
    icon is an optional custom icon to use for this file type
   */
  static void add_type(const char *extensions, const char *short_description,
                       Fl_Image *icon = NULL);
  ...
  //! Set the default icon to use for all files for which no other icon has been
  //! specified
  ...
  static void _qSort(int how, bool caseSort, Fl_Widget **array, int low,
                     int high);
  ...
  public:
    ...
    inline Fl_Widget *child(int n) const { return Flu_Wrap_Group::child(n); }
