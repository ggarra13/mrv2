...
Viewport::Viewport(int X, int Y, int W, int H, const char *L)
    : TimelineViewport(X, Y, W, H, L), _gl(new GLPrivate) {
...
void Viewport::_drawCursor(const math::Matrix4x4f &mvp) const noexcept {
  TLRENDER_GL();
  TLRENDER_P();
  if (p.actionMode != ActionMode::kScrub && p.actionMode != ActionMode::kText &&
...
void Viewport::_drawSphericalEnvironmentMap() {
  ...
  TLRENDER_GL();
  const auto &mesh =
      geom::createSphere(2.0F, p.environmentMapOptions.subdivisionX,
...
void Viewport::draw() {
  ...
  try {
    if (renderSize.isValid()) {
      ...
      if (gl::doCreate(gl.buffer, renderSize, offscreenBufferOptions)) {
        ...
        unsigned dataSize = renderSize.w * renderSize.h * 4 * sizeof(GLfloat);
  ...
  if (gl.buffer) {
    ...
    if (p.environmentMapOptions.type != EnvironmentMapOptions::kNone) {
      ...
      const glm::mat4x4 pm =
          glm::ortho(0.F, static_cast<float>(viewportSize.w), 0.F,
...
void Viewport::_drawAnnotationsGL2() {
  ...
  if (!annotations.empty()) {
    ...
    for (const auto &annotation : annotations) {
      ...
      auto mvp = math::Matrix4x4f(vm[0][0], vm[0][1], vm[0][2], vm[0][3],
...
inline void
Viewport::_drawText(const std::vector<std::shared_ptr<imaging::Glyph>> &glyphs,
...
void Viewport::_calculateColorArea(area::Info &info) {
  ...
  info.rgba.mean.r = info.rgba.mean.g = info.rgba.mean.b = info.rgba.mean.a =
      0.F;
  ...
  info.hsv.mean.r = info.hsv.mean.g = info.hsv.mean.b = info.hsv.mean.a = 0.F;
...
void Viewport::_drawSafeAreas(const float percentX, const float percentY,
...
void Viewport::_drawSafeAreas() const noexcept {
  ...
  vm = glm::translate(vm, glm::vec3(p.viewPos.x, p.viewPos.y, 0.F));
  vm = glm::scale(vm, glm::vec3(p.viewZoom, p.viewZoom, 1.F));
  glm::mat4x4 pm = glm::ortho(0.F, static_cast<float>(viewportSize.w), 0.F,
                              static_cast<float>(viewportSize.h), -1.F, 1.F);
  glm::mat4x4 vpm = pm * vm;
  vpm = glm::scale(vpm, glm::vec3(1.F, -1.F, 1.F));
  auto mvp = math::Matrix4x4f(vpm[0][0], vpm[0][1], vpm[0][2], vpm[0][3],
                              vpm[1][0], vpm[1][1], vpm[1][2], vpm[1][3],
                              vpm[2][0], vpm[2][1], vpm[2][2], vpm[2][3],
                              vpm[3][0], vpm[3][1], vpm[3][2], vpm[3][3]);
...
void Viewport::_drawHUD() const noexcept {
  ...
  if (p.hud & HudDisplay::kDirectory) {
    ...
    _drawText(p.fontSystem->getGlyphs(directory, fontInfo), pos, lineHeight,
              labelColor);
  }
  ...
  if (p.hud & HudDisplay::kResolution) {
    ...
    if (video.size.pixelAspectRatio != 1.0) {
      ...
      snprintf(buf, 512, "%d x %d  ( %.3g )  %d x %d", video.size.w,
               video.size.h, video.size.pixelAspectRatio, width, video.size.h);
    } else {
      snprintf(buf, 512, "%d x %d", video.size.w, video.size.h);
    }
    _drawText(p.fontSystem->getGlyphs(buf, fontInfo), pos, lineHeight,
              labelColor);
  }
  ...
  if (p.hud & HudDisplay::kFrameRange) {
    ...
    snprintf(buf, 512, "Range: %" PRId64 " -  %" PRId64, frame, last_frame);
    tmp += buf;
  }
  ...
  if (!tmp.empty())
    _drawText(p.fontSystem->getGlyphs(tmp, fontInfo), pos, lineHeight,
              labelColor);
  ...
  if (p.hud & HudDisplay::kAttributes) {
    ...
    for (const auto &tag : info.tags) {
      if (pos.y > viewportSize.h)
        return;
      snprintf(buf, 512, "%s = %s", tag.first.c_str(), tag.second.c_str());
      _drawText(p.fontSystem->getGlyphs(buf, fontInfo), pos, lineHeight,
                labelColor);
    }
  }
