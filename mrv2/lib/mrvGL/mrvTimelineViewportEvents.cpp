...
void TimelineViewport::_handleDragLeftMouseButton() noexcept {
  ...
  if (p.compareOptions.mode == timeline::CompareMode::Wipe) {
    ...
    if (p.actionMode == ActionMode::kScrub ||
      ...
      return;
    } else if (Fl::event_shift() || p.actionMode == ActionMode::kSelection) {
      ...
      case ActionMode::kArrow: {
        ...
        double tPointOnLine = nWidth / (2 * (tanf(theta) / 2) * lineLength);
...
int TimelineViewport::acceptMultilineInput() noexcept {
  ...
  if (text && strlen(text) > 0) {
    ...
    auto pos = math::Vector2i(w->x() + offset.x, h() - w->y() - offset.y);
...
void TimelineViewport::handleViewSpinning() noexcept {
  ...
  if (changed) {
    ...
    Fl::repeat_timeout(kSpinTimeout, (Fl_Timeout_Handler)_handleViewSpinning_cb,
                       this);
  }
}
  ...
  case FL_RELEASE: {
    if (p.actionMode == ActionMode::kScrub ||
      ...
      if (p.lastEvent == FL_DRAG) {
        ...
        if (p.lastEvent == FL_PUSH && Fl::event_button() == FL_LEFT_MOUSE) {
          togglePlayback();
        }
      }
    }
  ...
  case FL_DRAG: {
    ...
    if (Fl::event_button1()) {
      ...
      if (Fl::event_alt()) {
        ...
        setViewZoom(viewZoom() + dx * viewZoom() / 500.0f, p.viewPosMousePress);
...
void TimelineViewport::dragAndDrop(const std::string &text) noexcept {
  ...
  for (auto file : tmpFiles) {
    ...
    if (mrv::is_directory(file.c_str())) {
      ...
      loadFiles.insert(loadFiles.end(), sequences.begin(), sequences.end());
