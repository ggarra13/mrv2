...
TimelineViewport::TimelineViewport(int X, int Y, int W, int H, const char *L)
    : Fl_SuperClass(X, Y, W, H, L), _p(new Private) {}

TimelineViewport::TimelineViewport(int W, int H, const char *L)
    : Fl_SuperClass(W, H, L), _p(new Private) {}
...
void TimelineViewport::scrub() noexcept {
  ...
  DBGM2("dx= " << dx << " X=" << X << " p.mousePress.x=" << p.mousePress.x
               << " t= " << t << " seek " << time);
  player->seek(time);
  p.mousePress.x = X;
}
...
void TimelineViewport::setLUTOptions(
    const timeline::LUTOptions &value) noexcept {
  TLRENDER_P();
...
const timeline::CompareOptions &TimelineViewport::getCompareOptions() noexcept {
...
bool TimelineViewport::hasFrameView() const noexcept { return _p->frameView; }

//! Return the safe areas status
bool TimelineViewport::getSafeAreas() const noexcept { return _p->safeAreas; }
...
HudDisplay TimelineViewport::getHudDisplay() const noexcept { return _p->hud; }
...
void TimelineViewport::currentVideoCallback(
  ...
  TLRENDER_P();
  const auto i =
      std::find(p.timelinePlayers.begin(), p.timelinePlayers.end(), sender);
...
void TimelineViewport::_updatePixelBar(imaging::Color4f &rgba) const noexcept {
  TLRENDER_P();
...
void TimelineViewport::updateColorConfigOptions() noexcept {
  ...
  p.colorConfigOptions.fileName = p.ui->uiPrefs->uiPrefsOCIOConfig->value();
...
void TimelineViewport::toggleDisplayChannel(const timeline::Channels &channel,
                                            int idx) noexcept {
  TLRENDER_P();
  timeline::DisplayOptions d;
  if (idx < 0)
...
void TimelineViewport::_mapBuffer() const noexcept {
  ...
  for (int Y = 0; Y < maxY; ++Y) {
    for (int X = 0; X < maxX; ++X) {
      imaging::Color4f &rgba = (imaging::Color4f &)p.image[(X + maxX * Y) * 4];
      rgba.r = rgba.g = rgba.b = rgba.a = 0.f;
