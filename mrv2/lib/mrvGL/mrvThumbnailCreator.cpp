...
void ThumbnailCreator::initThread() {
  ...
  Fl::add_timeout(p.timerInterval, (Fl_Timeout_Handler)timerEvent_cb, this);
}

int64_t ThumbnailCreator::request(
    const std::string &fileName, const otime::RationalTime &time,
    const imaging::Size &size, const callback_t callback, void *callbackData,
...
int64_t ThumbnailCreator::request(
    const std::string &fileName, const std::vector<otime::RationalTime> &times,
    const imaging::Size &size, const callback_t callback, void *callbackData,
    const timeline::ColorConfigOptions &colorConfigOptions,
    const timeline::LUTOptions &lutOptions) {
  TLRENDER_P();
...
void ThumbnailCreator::run() {
  ...
  if (auto context = p.context.lock()) {
    ...
    while (p.running) {
      ...
      // std::cout << "requests in progress: " << p.requestsInProgress.size() <<
      // std::endl; std::cout << "results: " << p.results.size() << std::endl;
      ...
      std::list<Private::Request> newRequests;
      {
        ...
        if (p.cv.wait_for(lock, p.requestTimeout, [this] {
              return !_p->requests.empty() || !_p->requestsInProgress.empty() ||
          ...
          while (!p.requests.empty() && (p.requestsInProgress.size() +
      ...
      for (auto &request : newRequests) {
        ...
        try {
          request.timeline =
              timeline::Timeline::create(request.fileName, context, options);
      ...
      while (requestIt != p.requestsInProgress.end()) {
        ...
        while (futureIt != requestIt->futures.end()) {
          if (futureIt->valid() && futureIt->wait_for(std::chrono::seconds(
                                       0)) == std::future_status::ready) {
            const int depth = 4;
            const auto videoData = futureIt->get();
            const imaging::Info info(requestIt->size.w, requestIt->size.h,
                                     imaging::PixelType::RGBA_U8);
            uint8_t *pixelData =
                new uint8_t[static_cast<size_t>(info.size.w) *
            ...
            try {
              ...
              if (gl::doCreate(offscreenBuffer, info.size,
                ...
                offscreenBuffer = gl::OffscreenBuffer::create(
              ...
              glReadPixels(0, 0, info.size.w, info.size.h, GL_RGBA,
                           GL_UNSIGNED_BYTE, pixelData);
            } catch (const std::exception &e) {
              std::cerr << e.what() << std::endl;
              context->log(kModule, e.what(), log::Type::Error);
            }

            const auto flImage =
                new Fl_RGB_Image(pixelData, info.size.w, info.size.h, depth);
...
void ThumbnailCreator::timerEvent() {
  ...
  if (p.running) {
    Fl::repeat_timeout(p.timerInterval, (Fl_Timeout_Handler)timerEvent_cb,
                       this);
  }
}
