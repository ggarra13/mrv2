...
OffscreenContext::OffscreenContext() : _p(new Private) {}
...
void OffscreenContext::make_current() {
  ...
  if (p.dpy) {
    ...
    const int fbCfgAttribslist[] = {GLX_RENDER_TYPE,
                                    GLX_RGBA_BIT,
                                    GLX_DRAWABLE_TYPE,
                                    GLX_PBUFFER_BIT,
                                    GLX_RED_SIZE,
    ...
    const int pfbCfg[] = {GLX_PBUFFER_WIDTH, 1, GLX_PBUFFER_HEIGHT, 1, None};
    ...
    p.x11_context =
        glXCreateNewContext(p.dpy, glxfbCfg[0], GLX_RGBA_TYPE, NULL, GL_TRUE);
    if (!p.x11_context) {
  ...
  if (p.wld) {
    ...
    p.egl_surface = eglCreatePbufferSurface(p.egl_display, egl_config, pfbCfg);
    ...
    p.egl_context =
        eglCreateContext(p.egl_display, egl_config, EGL_NO_CONTEXT, NULL);
...
#if defined(FLTK_USE_WAYLAND)
  if (p.wld) {
    if (!eglMakeCurrent(p.egl_display, EGL_NO_SURFACE, EGL_NO_SURFACE,
                        EGL_NO_CONTEXT)) {
      LOG_ERROR("Could not make the null context current");
      LOG_ERROR(eglGetErrorString(eglGetError()));
    }
