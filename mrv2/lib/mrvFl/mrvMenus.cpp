...
void MainWindow::fill_menu(Fl_Menu_ *menu) {
  ...
  DBG3;
  idx = menu->add(_("File/Open/Movie or Sequence"), kOpenImage.hotkey(),
  ...
  idx =
      menu->add(_("File/Open/With Separate Audio"), kOpenSeparateAudio.hotkey(),
  ...
  menu->add(_("File/Save/Movie"), kSaveSequence.hotkey(),
  ...
  idx = menu->add(_("File/Close Current"), kCloseCurrent.hotkey(),
                  (Fl_Callback *)close_current_cb, ui, mode);

  idx = menu->add(_("File/Close All"), kCloseAll.hotkey(),
  ...
  for (auto file : recentFiles) {
    ...
    while ((pos = file.find('/', pos)) != std::string::npos) {
      file = file.substr(0, pos) + "\\" + file.substr(pos, file.size());
      pos += 2;
    }
  ...
  menu->add(_("File/Quit"), kQuitProgram.hotkey(), (Fl_Callback *)exit_cb, ui);
  ...
  idx = menu->add(buf, kToggleMenuBar.hotkey(), (Fl_Callback *)toggle_menu_bar,
                  ui, FL_MENU_TOGGLE);
  item = (Fl_Menu_Item *)&(menu->menu()[idx]);
  ...
  idx = menu->add(buf, kToggleTopBar.hotkey(), (Fl_Callback *)toggle_top_bar,
                  ui, FL_MENU_TOGGLE);
  item = (Fl_Menu_Item *)&(menu->menu()[idx]);
  ...
  idx = menu->add(buf, kTogglePixelBar.hotkey(),
                  (Fl_Callback *)toggle_pixel_bar, ui, FL_MENU_TOGGLE);
  item = (Fl_Menu_Item *)&(menu->menu()[idx]);
  ...
  idx = menu->add(buf, kToggleTimeline.hotkey(),
                  (Fl_Callback *)toggle_bottom_bar, ui, FL_MENU_TOGGLE);
  item = (Fl_Menu_Item *)&(menu->menu()[idx]);
  ...
  idx = menu->add(buf, kToggleStatusBar.hotkey(),
                  (Fl_Callback *)toggle_status_bar, ui, FL_MENU_TOGGLE);
  item = (Fl_Menu_Item *)&(menu->menu()[idx]);
  ...
  idx = menu->add(buf, kToggleToolBar.hotkey(),
                  (Fl_Callback *)toggle_action_tool_bar, ui, FL_MENU_TOGGLE);
  item = (Fl_Menu_Item *)&(menu->menu()[idx]);
  if (ui->uiToolsGroup->visible())
    item->set();

  idx = menu->add(_("Panel/One Panel Only"), kToggleOnePanelOnly.hotkey(),
  ...
  for (; wc->name; ++wc) {
    ...
    int idx = menu->add(menu_name.c_str(), hotkey, (Fl_Callback *)window_cb, ui,
                        FL_MENU_TOGGLE);
  ...
  if (d.mirror.x)
    mode |= FL_MENU_VALUE;
  idx = menu->add(_("Render/Mirror X"), kFlipX.hotkey(),
  ...
  if (d.mirror.y)
    mode |= FL_MENU_VALUE;
  idx = menu->add(_("Render/Mirror Y"), kFlipY.hotkey(),
  ...
  idx = menu->add(_("Render/Magnify Filter/Linear"), kTextureFiltering.hotkey(),
  ...
  idx = menu->add(_("Playback/Stop"), kStop.hotkey(), (Fl_Callback *)stop_cb,
                  ui, mode);
  item = (Fl_Menu_Item *)&(menu->menu()[idx]);
  ...
  idx = menu->add(_("Playback/Loop Playback"), kPlaybackLoop.hotkey(),
                  (Fl_Callback *)playback_loop_cb, ui, mode);
  item = (Fl_Menu_Item *)&(menu->menu()[idx]);
  if (loop == timeline::Loop::Loop)
    item->set();
  idx = menu->add(_("Playback/Playback Once"), kPlaybackOnce.hotkey(),
                  (Fl_Callback *)playback_once_cb, ui, mode);
  item = (Fl_Menu_Item *)&(menu->menu()[idx]);
  if (loop == timeline::Loop::Once)
    item->set();
  idx = menu->add(_("Playback/Playback Ping Pong"), kPlaybackPingPong.hotkey(),
  ...
  menu->add(_("Playback/Go to/Start"), 0, (Fl_Callback *)start_frame_cb, ui,
            mode);
  menu->add(_("Playback/Go to/End"), 0, (Fl_Callback *)end_frame_cb, ui,
            FL_MENU_DIVIDER | mode);

  menu->add(_("Playback/Go to/Previous Frame"), kFrameStepBack.hotkey(),
            (Fl_Callback *)previous_frame_cb, ui, mode);
  menu->add(_("Playback/Go to/Next Frame"), kFrameStepFwd.hotkey(),
  ...
  if (player) {
    ...
    if (!annotations.empty()) {
      ...
      menu->add(_("Playback/Go to/Next Annotation"),
                kShapeFrameStepFwd.hotkey(), (Fl_Callback *)next_annotation_cb,
                ui, FL_MENU_DIVIDER | mode);
      menu->add(_("Playback/Annotation/Clear"), kShapeFrameClear.hotkey(),
  ...
  idx = menu->add(buf, kHudToggle.hotkey(), (Fl_Callback *)hud_toggle_cb, ui,
                  mode);
  item = (Fl_Menu_Item *)&(menu->menu()[idx]);
  ...
  if (numFiles > 0 && aIndex >= 0 && aIndex < numFiles) {
    ...
    if (!ioInfo.video.empty()) {
      {
        ss << "V:" << ioInfo.video[0].size.w << "x" << ioInfo.video[0].size.h
    ...
    if (ioInfo.audio.isValid()) {
      ...
      ss << "A: " << static_cast<size_t>(ioInfo.audio.channelCount) << " "
         << ioInfo.audio.dataType << " " << ioInfo.audio.sampleRate;
    }
    char buf[256];
  ...
  if (numFiles > 0) {
    ...
    if (has_version) {
      menu->add(_("Image/Version/First"), kFirstVersionImage.hotkey(),
                (Fl_Callback *)first_image_version_cb, ui);
      menu->add(_("Image/Version/Last"), kLastVersionImage.hotkey(),
                (Fl_Callback *)last_image_version_cb, ui, FL_MENU_DIVIDER);
      menu->add(_("Image/Version/Previous"), kPreviousVersionImage.hotkey(),
                (Fl_Callback *)previous_image_version_cb, ui);
      menu->add(_("Image/Version/Next"), kNextVersionImage.hotkey(),
