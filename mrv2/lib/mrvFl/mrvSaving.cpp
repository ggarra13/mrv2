...
void save_movie(const std::string &file, ViewerUI *ui) {
  ...
  if (originalFile == file) {
    throw std::runtime_error(
        string::Format("{0}: Saving over same file being played!").arg(file));
  }
  ...
  auto buffer = gl::OffscreenBuffer::create(renderSize, offscreenBufferOptions);
  ...
  while (running) {
    ...
    glReadPixels(0, 0, outputInfo.size.w, outputInfo.size.h, format, type,
                 outputImage->getData());
