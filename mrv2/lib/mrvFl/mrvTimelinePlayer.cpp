...
void TimelinePlayer::_init(
  ...
  p.currentTimeObserver = observer::ValueObserver<otime::RationalTime>::create(
      ...
      [this](const otime::RationalTime &value) { currentTimeChanged(value); });

  p.inOutRangeObserver = observer::ValueObserver<otime::TimeRange>::create(
      p.timelinePlayer->observeInOutRange(),
      [this](const otime::TimeRange value) { inOutRangeChanged(value); });
...
#ifdef DEBUG_SPEED
        std::cout << ">>>>>>> currentVideoTime = " << value.time << std::endl;
#endif
...
void TimelinePlayer::resetOutPoint() { _p->timelinePlayer->resetOutPoint(); }
...
void TimelinePlayer::setMute(bool value) { _p->timelinePlayer->setMute(value); }
...
TimelinePlayer::getAnnotations(const int previous, const int next) const {
  ...
  while (found != p.annotations.end()) {
    found = std::find_if(found, p.annotations.end(),
...
std::shared_ptr<tl::draw::Annotation> TimelinePlayer::getAnnotation() {
  ...
  auto found =
      std::find_if(p.annotations.begin(), p.annotations.end(),
...
TimelinePlayer::createAnnotation(const bool all_frames) {
  ...
  if (found == p.annotations.end()) {
    auto annotation = std::make_shared<draw::Annotation>(frame, all_frames);
