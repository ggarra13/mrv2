...
static void toggle_channel(Fl_Menu_Item *item, TimelineViewport *view,
...
void about_cb(Fl_Widget *w, ViewerUI *ui) { show_window_cb(_("About"), ui); }
...
// Playback callbacks
void play_forwards_cb(Fl_Menu_ *, ViewerUI *ui) { ui->uiView->playForwards(); }
...
void create_playlist(
    ViewerUI *ui, const std::vector<std::shared_ptr<FilesModelItem>> &items) {
  ...
  try {
    ...
    const auto &timeline = timeline::create(items, ui->app->getContext());
    otio::ErrorStatus errorStatus;
    bool ok = timeline->to_json_file(otioFileName, &errorStatus);
    if (!ok || otio::is_error(errorStatus)) {
      std::string error = "Could not save EDL: " + errorStatus.full_description;
