 ...
 */
int environmentSetting(const char *variable, const int defaultValue,
                       const bool inPrefs) {
  int r = defaultValue;
 ...
 *         to the default value.
 */
float environmentSetting(const char *variable, const float defaultValue,
                         const bool inPrefs) {
 ...
 *         to the default value.
 */
const char *environmentSetting(const char *variable, const char *defaultValue,
                               const bool inPrefs) {
  const char *env = fl_getenv(variable);
  if (!env || strlen(env) == 0) {
    env = defaultValue;
...
static std::string expandVariables(const std::string &s,
  ...
  return expandVariables(pre + value + post, START_VARIABLE, END_VARIABLE);
}
...
Preferences::Preferences(PreferencesUI *uiPrefs, bool reset) {
  ...
  for (unsigned i = 0; i < num; ++i) {
    ...
    if (key[1] == '#') {
      ...
      case 's':
        ...
        break;
      case 'v':
        // void values are not cleared nor stored as that can corrupt the
        // prefs.
        continue;
        break;
      default:
  ...
  if (loaded) {
    std::string msg = tl::string::Format(_("Loaded color themes from \"{0}\"."))
                          .arg(colorname);
  ...
  if (language && strlen(language) > 1) {
    for (unsigned i = 0; i < sizeof(kLanguages) / sizeof(LanguageTable); ++i) {
  ...
  if (!var || strlen(var) == 0 || reset) {
    mrvLOG_INFO("ocio", _("Setting OCIO config to nuke-default.") << std::endl);
...
#define OCIO_ICS(x, d)                                                         \
  ...
  keys =
      new Fl_Preferences(prefspath().c_str(), "filmaura", hotkeys_file.c_str());
...
void Preferences::save() {
  ...
  gui.set("timeline_thumbnails", uiPrefs->uiPrefsTimelineThumbnails->value());
  ...
  if (!fs::exists(prefspath() + hotkeys_file)) {
    Fl_Preferences keys(prefspath().c_str(), "filmaura", hotkeys_file.c_str());
    save_hotkeys(keys);
  }
  ...
  std::string msg = tl::string::Format(_("Preferences have been saved to: "
                                         "\"{0}mrv2.prefs\"."))
                        .arg(prefspath());
  LOG_INFO(msg);
...
void Preferences::run(ViewerUI *m) {
  ...
  if (uiPrefs->uiPrefsMacOSMenus->value()) {
    ...
    Fl_Sys_Menu_Bar *smenubar = dynamic_cast<Fl_Sys_Menu_Bar *>(ui->uiMenuBar);
    if (smenubar) {
      smenubar->clear();
      delete ui->uiMenuBar;
  ...
  if (var && strlen(var) > 0) {
    ...
    try {
      ...
      if (viewlist) {
        ...
        for (unsigned i = 0; i < active_views.size(); ++i) {
          while (active_views[i][0] == ' ')
            active_views[i] = active_views[i].substr(1, active_views[i].size());
        }
      }
      ...
      for (size_t j = 0; j < num_active_displays; ++j) {
        ...
        size_t pos = 0;
        while ((pos = quoted_display.find('/', pos)) != std::string::npos) {
