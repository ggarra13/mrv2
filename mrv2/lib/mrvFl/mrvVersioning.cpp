...
const boost::regex version_regex(const ViewerUI *ui, const bool verbose) {
  ...
  try {
    ...
    std::string msg =
        tl::string::Format(_("Regular expression error: {0}")).arg(e.what());
    LOG_ERROR(msg);
  }
...
std::string media_version(const ViewerUI *ui, const file::Path &path, int sum,
  ...
  while ((first_or_last || start == AV_NOPTS_VALUE) && tries <= max_tries) {
    ...
    try {
      ...
      while (boost::regex_search(tstart, tend, what, expr, flags)) {
        ...
        std::string msg =
            tl::string::Format(_("Iteration {0} matched prefix={1}"))
                .arg(iter)
                .arg(prefix);
        LOG_INFO(msg);
        msg = tl::string::Format(_("Iteration {0} matched version={1}"))
                  .arg(iter)
                  .arg(number);
        LOG_INFO(msg);
        msg = tl::string::Format(_("Iteration {0} matched suffix={1}"))
                  .arg(iter)
                  .arg(suffix);
        LOG_INFO(msg);
        LOG_INFO("-------------------------------------------------------------"
        ...
        if (!number.empty()) {
          ...
          msg = tl::string::Format(_("Iteration {0} will check version={1}"))
                    .arg(iter)
                    .arg(buf);
          LOG_INFO(msg);
          newfile += buf;
