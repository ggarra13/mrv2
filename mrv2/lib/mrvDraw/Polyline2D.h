...
public:
  ...
  enum class EndCapStyle {
     ...
     * When using this EndCapStyle, don't specify the common start/end point
     * twice, as Polyline2D connects the first and last input point itself.
     */
    JOINT
  };

  /**
   * Creates a vector of vertices describing a solid path through the input
   * points.
   ...
   *                                         This yields better results when
   * dealing with paths whose points have a distance smaller than the thickness,
   *                                         but may introduce overlapping
  ...
  static std::vector<Point> create(const InputCollection &points,
                                   float thickness,
  ...
  static std::vector<Point> create(const std::vector<Point> &points,
                                   float thickness,
                                   JointStyle jointStyle = JointStyle::MITER,
                                   EndCapStyle endCapStyle = EndCapStyle::BUTT,
                                   bool allowOverlap = false) {
    std::vector<Point> vertices;
    create<Point, std::vector<Point>>(vertices, points, thickness, jointStyle,
                                      endCapStyle, allowOverlap);
    return vertices;
  }

  template <typename Point, typename InputCollection>
  static size_t create(std::vector<Point> &vertices,
    ...
    create<Point, InputCollection>(std::back_inserter(vertices), points,
                                   thickness, jointStyle, endCapStyle,
                                   allowOverlap);
  ...
  static OutputIterator create(OutputIterator vertices,
                               const InputCollection &points, float thickness,
                               JointStyle jointStyle = JointStyle::MITER,
                               EndCapStyle endCapStyle = EndCapStyle::BUTT,
                               bool allowOverlap = false) {
    ...
    if (endCapStyle == EndCapStyle::SQUARE) {
      ...
      createJoint(vertices, lastSegment, firstSegment, jointStyle, pathEnd1,
                  pathEnd2, pathStart1, pathStart2, allowOverlap);
    }
    ...
    for (size_t i = 0; i < segments.size(); i++) {
      ...
      if (i + 1 == segments.size()) {
        ...
        createJoint(vertices, segment, segments[i + 1], jointStyle, end1, end2,
...
private:
  ...
  createJoint(OutputIterator vertices, const PolySegment<Point> &segment1,
    ...
    if (jointStyle == JointStyle::MITER) {
      ...
      auto sec1 = LineSegment<Point>::intersection(segment1.edge1,
                                                   segment2.edge1, true);
      auto sec2 = LineSegment<Point>::intersection(segment1.edge2,
                                                   segment2.edge2, true);
      ...
      auto innerSecOpt =
          LineSegment<Point>::intersection(*inner1, *inner2, allowOverlap);
      ...
      if (jointStyle == JointStyle::BEVEL) {
        ...
        *vertices++ = innerSec;
        ...
        createTriangleFan(vertices, innerSec, segment1.center.b, outer1->b,
                          outer2->a, clockwise);
      } else {
        assert(false);
      }
  ...
  template <typename Point, typename OutputIterator>
  static OutputIterator
  createTriangleFan(OutputIterator vertices, Point connectTo, Point origin,
    ...
    auto numTriangles =
        std::max(1, (int)std::floor(std::abs(jointAngle) / roundMinAngle));
