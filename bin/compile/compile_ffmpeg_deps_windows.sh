...
#
# Thi script if TLRENDER_NET is on, downloads openssl and libcrypto from MSys2
# and copies them so they can be used by FFmpeg on Windows MSys2 build.

#
# \@note: thiis script is outdated and should no longer be used, as vcpkg now
#         handles this.
#
...
# 	    run_cmd cp /ucrt64/lib/libssl.dll.a $INSTALL_DIR/lib/libssl.lib
# 	    run_cmd cp /ucrt64/bin/libcrypto*.dll $INSTALL_DIR/bin/
# 	    run_cmd cp /ucrt64/lib/libcrypto.dll.a $INSTALL_DIR/lib/libcrypto.lib
...
# 	   #### don't do this: run_cmd sed -i -e 's%Requires.private:.libcrypto%%' $INSTALL_DIR/lib/pkgconfig/openssl.pc
